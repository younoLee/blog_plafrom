...
# ── 실패 개수를 지표로 내보낸다 (2026-09-07 추가) ──────────────────────────
#
# **왜 필요한가.** 이 파일의 워크플로(`watch.yml`) 머리말은 "실패하면 GitHub이 메일로
# 알려준다"고 적어두고 그걸 알림 경로로 삼고 있었다. 그런데 2026-09-06 밤에 이 감시가
# 빨간불이 됐는데 **다음 날 사람이 Actions 탭을 직접 열어보기 전까지 아무도 몰랐다.**
# 즉 그 문장이 사실인지 아무도 확인한 적이 없었다. 이 저장소가 "발동했다와 닿았다는
# 다르다"(07-30 비용 훈련)라고 이름 붙인 자리이고, 정작 그 결론을 **감시 자신에게는
# 적용하지 않고 있었다.**
#
# **왜 이 방식인가.** 새 경로를 만들지 않고 **이미 사람에게 닿는 것이 확인된 경로**에
# 얹는다. 바로 위 하트비트가 쓰는 `blog/watch` 네임스페이스이고, 역할 권한도
# 네임스페이스 단위라(`PutWatchHeartbeat`) **IAM 변경이 필요 없다.** 그 SNS 토픽은
# 이 스크립트가 매 실행 "알람 전달 경로 정상 (확인된 구독 N개)"으로 되짚고 있다.
#
# 값은 `FAIL + WARN` 이다 — 마지막 줄의 종료코드와 **같은 식**이라야 "Actions 는 빨간데
# 알람은 조용" 같은 어긋남이 안 생긴다. 그래서 경고만 있어도 값이 올라간다(의도된 것:
# 09-06 의 빨간불도 경고 1건이었다).
#
# **0 도 반드시 보낸다.** 안 보내면 '문제 없음'과 '감시가 안 돎'이 CloudWatch 에서
# 똑같이 '데이터 없음'이 되고, 그건 이 스크립트가 반복해 갈라온 두 상태다.
# 침묵의 의미는 하트비트 알람이 따로 맡는다(그쪽은 treat_missing_data=breaching).
PROBLEMS=$((FAIL + WARN))
if aws cloudwatch put-metric-data --namespace "blog/watch" \
  --metric-name Problems --value "$PROBLEMS" --unit Count --region "$REGION" 2>/tmp/pb.err; then
  echo "  --   문제 개수 발행됨 (blog/watch Problems=$PROBLEMS)"
else
  # 여기서 warn 을 올려도 **이미 보낸 값에는 안 들어간다**(순서상 그렇다). 그래도 올린다 —
  # 종료코드가 빨개져서 Actions 쪽은 빨간불이 되고, 다음 실행이 0 이 아닌 값을 보낸다.
  warn "문제 개수를 못 보냈다 — 이번 회차는 blog-watch-problems 알람이 못 본다."
  sed 's/^/       /' /tmp/pb.err
fi
