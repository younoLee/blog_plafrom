...
- **크레딧 수명 ($75.10 / 2027-06-24 만료 기준)** — **뭐가 먼저 바닥나는지가 사용 패턴에 따라 뒤바뀐다**:
  - **정지 유지**(월 ~$3.7, 스토리지만): 만료일까지 $42만 쓰고 **$33이 남은 채 소멸**. → 제약은 **만료일**이지 잔액이 아니다.
  - **24/7 가동**(월 ~$32): **2.3개월 = ~2026-09-26에 소진**. → 제약은 **잔액**.
  - (Explore $60까지 유효하면 24/7이 4.2개월 → ~11-22. 하지만 미확인이라 계획엔 안 넣는다.)
...
- **확인할 것**: Explore 크레딧 3개($60)의 **적용 대상 서비스**(콘솔 → 크레딧 → '전체 서비스 목록 보기'). EC2·RDS가 목록에 있으면 수명이 2.3 → 4.2개월로 늘어난다. 전체 크레딧의 44%라 확인할 값어치가 있다.
