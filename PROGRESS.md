...
- **`github-brench` 정책을 terraform으로 회수**: 콘솔에서 만들어져 **terraform 밖**에 있던 마지막 리소스. 즉 프론트 배포가 무슨 권한으로 도는지가 저장소 어디에도 없었다. 오늘 사고(`blog-ec2-role`이 조용히 교체돼 이미지 업로드가 AccessDenied)와 **정확히 같은 클래스**라 남겨둘 이유가 없었다. `terraform import` 후 **plan 무변경** — 정의가 실제 정책과 한 글자도 다르지 않다는 증거다(권한을 건드리지 않고 관리만 가져왔다). 관리 리소스 22 → 24개.
