...
```
accountPlanType: PAID / accountPlanStatus: ACTIVE / 남은 크레딧: $115.10
...
- **크레딧 수명**: 24/7이면 월 ~$32 → **~3.6개월(2026년 11월 초 소진)**. 정지해두면 월 ~$3.7(스토리지만) → **~2년 반**. `PAID` 플랜이라 **크레딧이 0이 되는 순간 카드로 진짜 청구가 시작된다.**
...
- **남은 것**: **크레딧 만료일 확인**(소진 전에 날짜로 만료될 수 있음 — API엔 안 나와 빌링 콘솔을 봐야 함). **RDS→EC2 이전이 이제 '언젠가'가 아니라 11월 전에 해야 하는 일**(크레딧 소진 = 실청구 시작). 실결제 전환, 커스텀 도메인.
