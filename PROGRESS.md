...
- 메모: 콘솔에서 끄려면 배포 보안탭 "Manage protections"인데 새 UI에서 못 찾음. 나중 Terraform 땐 WAF를 안 켜거나 업로드 경로 예외로 코드화. <ins>(**2026-08-11 완료** — 끄는 건 불가로 판명(Free 요금제가 WebACL을 필수로 요구), 대신 `terraform/waf.tf`로 **override를 코드화**했다.)</ins>
...
- **⚠️ 배포 순서(중요)**: `git push` = **프론트만** 자동 배포. 프론트가 먼저 가면 새 프론트가 옛 백엔드의 없는 `/api/posts/meta`를 불러 **사이드바가 빈다**(목록은 배열 호환 처리해둬서 안 깨짐). → **백엔드 배포(재빌드=마이그레이션 자동) 먼저, 그다음 push.** <ins>(**2026-08-11 정정**: `git push`는 이제 아무것도 배포하지 않는다 — 프론트 자동 배포를 없앴다. 순서 규칙은 그대로고, 두 번째 단계가 push가 아니라 Actions의 `Run workflow`다.)</ins>
...
### 🧩 복잡도 hotspot 리팩터: create_draft (2026-07-18)

"복잡한 코드 찾아줘" → 추측 대신 **McCabe 복잡도로 측정**. 전체에서 임계값(8)을 넘긴 함수는 **딱 하나** — `ai.py:create_draft` **복잡도 16**(나머지는 전부 8 이하 = 코드가 대체로 잘 쪼개져 있음).
- 이 함수가 6가지(모델결정·티어권한·provider판별·비용캡·BYOK키로드·SSRF재검증·에러처리)를 한 몸에 처리 → 분기 폭증.
...
### 🔀 구독 UX 개편 1단계: 글쓴이별 독립 구독 (2026-07-18)

구독 로드맵을 잡고 그 **1단계**만 진행. 구상: ① (지금)구독=글쓴이별로 따로, 그 사람 '구독자공개' 글 열람 → ② (나중)글쓴이가 구독 신청 승인 → ③ (그 다음)선택적 알림.
- **문제**: 기존 UI가 "구독 이메일"(마스터 토글)을 먼저 켜야 글쓴이 버튼이 풀리는 **잠금 구조**. 게다가 그 글쓴이 버튼은 이름만 "새 글 알림"이지 실제론 **열람 권한**만 켰다(알림은 전체 뉴스레터가 따로 담당). 개념이 뒤엉켜 있었다.
...
### 🔔 인앱 알림: 헤더 종 + 안 읽음 배지 (2026-07-18)

이메일 알림에 더해 "화면에도 알려달라" → SNS식 인앱 알림. 승인+알림 켠 글쓴이가 새 글을 쓰면 헤더 종에 빨간 배지가 뜨고, 누르면 드롭다운 목록(글로 이동).
...
07-18 구독 3단계·인앱 알림은 **로컬 도커에서만** 검증하고 `git push`까지 했다. push = 프론트 자동 배포이므로(:915의 순서 규칙) **프론트만 새 코드, EC2 백엔드는 07-17 상태**로 어긋나 있다. 지금은 EC2가 주차/정지라(`/api/status` 무응답) 겉으로 드러난 사고는 없지만, **켜는 순간 깨진 화면이 뜬다.**

- **깨지는 것**: 새 프론트가 부르는 `/api/notifications`, `/api/subscriptions/requests`, `PUT /api/subscriptions/{id}/notify` 가 옛 백엔드엔 없음 → 헤더 종·구독 신청 화면 실패.
- **미적용 마이그레이션 3개**: `814d837c28a3`(notify) · `d29ba157b9a6`(approved, **기존 구독 백필 포함** — 이거 빠지면 기존 구독자가 열람 권한을 잃는다) · `2a2a9af10b7c`(notifications 테이블).
- **다음 세션 순서**: EC2 start → **백엔드 배포(재빌드=마이그레이션 자동)** → `/api/status` 확인 → 그다음에야 라이브 e2e. 프론트는 이미 나가 있으니 추가 push 불필요.
- **같이 처리할 것**: ① 개발일지 #15·#16 발행(`publish_devlogs.py`, 컨테이너 안에서 — 지금 연재는 14편에서 멈춰 있음) ② RDS 스냅샷 `blog-db-pre-migration-2026-07-18` 삭제 판단(이전 검증 완료, 보관 사유 소멸).
- **배운 것**: :915에 "백엔드 먼저, 그다음 push"라고 적어두고도 밟았다 — 규칙을 적는 것과 **작업 흐름에 끼워넣는 것**은 다르다. 로컬 도커 검증이 통과하면 '끝난 느낌'이 들어서 push가 반사적으로 나가는 게 원인. 서버가 꺼져 있어 즉시 드러나지 않는 것도 위험을 키웠다(조용한 어긋남).

### 🚀 밀린 배포 해소 + 일지 #15·#16 발행 + 스냅샷 정리 [완료/라이브검증] (2026-07-20)

위 어긋남을 실제로 해소한 세션. **켜기 → 배포 → 발행 → 정리 → 끄기**를 한 번에.

- **켜기**: EC2 start → 새 주소 `ec2-13-125-234-123` → `terraform apply -var=backend_origin_dns=...`(1 changed). RDS는 이전 완료라 켤 대상이 EC2뿐이다.
- **배포**: tar(app/alembic/alembic.ini/requirements/Dockerfile, `.env`·uploads 제외) → scp → `~/blog` 추출(`.env` 보존 확인) → 재빌드는 규칙7대로 사용자가 실행. **alembic head `51cafb80733f` → `2a2a9af10b7c`** = 마이그레이션 3개 적용.
- **데이터 무결성**: 배포 전 대조값을 먼저 세둔 뒤(구독 6·글 24·유저 5) 배포 후 재확인 — **구독 6, 그중 approved 6/6**(백필 성공 = 기존 구독자 접근 유지), notify 0(opt-in 기본값), 글 24·유저 5 그대로.
- **라이브 검증**: `/api/status` backend·database·mail ok. 새 엔드포인트 3개 전부 **401**(= 라우터 존재; 404면 미배포였다). 프론트 홈 200.
- **발행**: `#15`·`#16` 생성 2건. 소급 날짜(KST 09:00 → UTC 00:00)·public·태그 5개·글자수(8,352/6,547) 확인.
- **⚠️ 발행에서 드러난 누락**: `publish_devlogs.py`가 **`series`를 안 채운다** → 글은 보이는데 연재 네비가 `null`. DB에서 2건 백필해 **연재 16편** 복구(#16=16/16, #1=1/16 경계 확인)하고, 스크립트에도 `series` 설정을 추가해 재발을 막았다.
- **스냅샷**: `blog-db-pre-migration-2026-07-18` 삭제 → RDS 잔여 리소스 **0**.
- **끄기**: `terraform apply`(주차) **먼저** → stop. 검증값이 07-17 기준선과 동일 — EC2 stopped·PublicIp null, EIP 0, 프론트 200, `/api/status` **504**(fail closed).

**🔴 이 세션에서 발견한 진짜 문제 — 일일 백업이 한 번도 안 돌았다**

스냅샷을 지우기 전에 "RDS 자동백업을 대체한다"던 일일 S3 백업을 확인했더니, 버킷에 **07-18 파일 1개뿐**이고 `/var/log/blog-db-backup.log`는 **아예 없었다**(= 한 번도 실행 안 됨). 그 1개는 설치 때 수동 실행분이었다.

- **원인**: cron이 `0 18 * * *` = **UTC 18시 = KST 새벽 3시**. 작업은 낮에 하고 끝나면 EC2를 끄니, **예약 시각에 서버가 켜져 있는 날이 없다.** 스크립트 주석의 "DB가 바뀌는 건 EC2 켜졌을 때뿐이라 의미상 정확"은 맞는 말이지만, **바뀐 걸 담는 시각이 꺼져 있는 시간대**라 결과가 빈 백업이었다.
- **조치**: 스냅샷 삭제 전에 백업을 수동으로 한 번 떠서(`blog-2026-07-20-0157.sql.gz`, 333KB) **복원 가능성까지 확인**(gzip 무결성·11개 테이블·`notifications` 포함·`approved`/`notify` 컬럼·#16 글 존재). 그걸 확인한 뒤에야 스냅샷을 지웠다.
- **조치(같은 날 완료)**: cron 시각을 옮기는 걸로는 부족하다 — 서버가 꺼진 시간이 대부분이라 '몇 시로 옮겨도' 놓칠 수 있다. 백업의 올바른 자리는 **끄기 직전**이라, 정지 절차를 `scripts/stop_server.sh`로 묶었다(아래 절).
- **배운 것**: **백업은 "설정했다"가 아니라 "산출물이 쌓이고 있다"로 확인해야 한다.** cron 등록도, 스크립트도, IAM 권한도 다 정상이었는데 결과물이 2일치 0개였다 — 검증을 '설정 존재'에서 멈췄으면 마지막 사본(스냅샷)을 지우고 나서야 알았을 것이다. 그리고 **자동화의 스케줄은 인프라 상태와 함께 봐야 한다**: 24시간 켜진 서버를 전제로 짠 시각이, 필요할 때만 켜는 서버에선 '절대 안 오는 시각'이 된다.

### 🛑 정지 절차를 스크립트로: 백업 → 주차 → 정지 → 검증 (2026-07-20)

위에서 드러난 '백업 0건'의 해법. 시각을 옮기는 대신 **백업을 끄는 절차 안으로** 옮겼다 — DB가 바뀌는 건 EC2가 켜진 동안뿐이고, 끄면 다음에 켤 때까지 사본을 만들 기회가 없으니 '끄기 직전'이 유일하게 확실한 시점이다.

- **`scripts/stop_server.sh`**: ① pg_dump → S3 ② `terraform apply`(주차) ③ stop ④ 검증(EC2 stopped·PublicIp null, EIP 0, 홈 200, `/api/status` 504)을 한 번에. 순서가 안전에 직결돼(주차가 정지보다 먼저) 사람이 매번 기억하는 대신 코드에 고정했다.
- **백업 실패 시 정지하지 않는다**: 사본 없이 끄지 않으려는 의도적 중단. 대신 EC2가 켜진 채 남으므로(크레딧 소모) 메시지에 그 사실과 선택지(재시도 / `--skip-backup` 강행)를 같이 띄운다. 조용히 성공한 척하거나 조용히 끄는 것보다 낫다.
- **'성공'과 '산출물'을 따로 검증**: 스크립트 종료코드가 0이어도 S3 객체 수가 안 늘었으면 실패로 친다. 이번 사건의 교훈(설정은 정상인데 결과물이 0건)을 그대로 코드에 옮긴 것.
- **문서 동기화**: `terraform/variables.tf`의 절차 주석에도 백업 단계를 넣어, 주석과 스크립트가 어긋나지 않게 했다.
- **검증**: 처음엔 EC2가 정지 상태라 '이미 꺼져 있음' 분기만 확인했으나(상태 감지 → 주차 `No changes` → 종료), 아래 cron 제거 작업으로 EC2를 켠 김에 **본 경로를 끝까지 실행**했다 — 백업 산출물 `blog-2026-07-20-0226.sql.gz`(333KB, 버킷 객체 2→3) 생성, 주차 1 changed, 정지 완료, 검증 4종(stopped·PublicIp null / EIP 0 / 홈 200 / `/api/status` 504) 전부 기준선과 일치. `bash -n`·`terraform validate`도 통과.
- **배운 것**: 자동화가 안 도는 이유가 '설정 오류'가 아니라 **'전제와 실제 운영 패턴의 불일치'**일 수 있다. cron은 24시간 켜진 서버를 전제로 짠 물건인데, 이 서버는 필요할 때만 켠다 — 고칠 대상은 시각이 아니라 **트리거의 종류**(시간 기반 → 사건 기반)였다.

### 🗑️ 옛 백업 cron 제거 — 잘못된 안전감 걷어내기 (2026-07-20)

정지 절차에 백업이 들어간 뒤로 root crontab의 `0 18 * * * /usr/local/bin/blog-db-backup.sh`는 **한 번도 안 도는데 "매일 백업된다"는 인상만 주는** 물건이 됐다. 지워서 백업 경로를 하나로 만들었다.

- **cron만 삭제, 스크립트는 보존**: `/usr/local/bin/blog-db-backup.sh`는 `stop_server.sh`가 호출하므로 필수다. root crontab에서 해당 줄만 걸러내는 방식으로 제거(다른 항목이 생겨도 안전하게) → `no crontab for root`.
- **저장소엔 없던 물건**: crontab도 백업 스크립트도 terraform user_data가 아니라 **서버에 손으로 심은 것**이었다. 그래서 삭제는 커밋으로 표현되지 않고, 대신 cron이 있다고 전제하던 주석 4곳(`stop_server.sh`, `variables.tf`, `ec2.tf`, `db-backup.tf`)을 고쳤다.
- **부수 성과 — `stop_server.sh` 본 경로 첫 실전**: 이 작업 때문에 EC2를 켠 김에 스크립트로 껐다. 백업 산출물 생성(버킷 2→3)·주차·정지·검증 4종 전부 정상. 어제까지 '검증 안 된 자동화'였던 게 이번에 실측됐다.
- **배운 것**: **안 도는 자동화는 없는 것보다 나쁘다** — 없으면 백업을 직접 챙기지만, 있으면 챙겼다고 믿는다. 대체 경로를 만들었으면 옛 경로는 남겨두지 말고 지워야 "어느 쪽이 진짜인가"를 다시 묻지 않는다. 서버에 손으로 심은 설정은 저장소에 흔적이 없어 이런 정리 대상이 되기도 쉽다(코드 리뷰로 안 걸린다).

### 📦 백업 스크립트를 저장소로 — 서버에만 있던 물건 회수 (2026-07-20)

cron을 지우며 드러난 문제: `/usr/local/bin/blog-db-backup.sh`가 **서버에만 손으로 심겨 있어** 버전 관리도 안 되고, 인스턴스를 새로 만들면 백업 능력이 조용히 사라졌다. `scripts/blog-db-backup.sh`로 회수했다.

- **저장소가 원본, 서버 사본은 파생**: `stop_server.sh`가 백업 단계에서 매번 저장소 판본을 scp → `install -m 755`로 덮어쓴 뒤 실행한다. 서버에서 누가 고쳐도 다음 실행에 되돌아오고, 새 인스턴스도 자동으로 능력을 갖춘다.
- **로직 무변경**: 서버 원본과 실행 라인을 대조해 동일함을 확인(주석·shebang만 추가).
- **검증(실측)**: EC2를 켜서 끝까지 확인했다. ① 교체 전 서버 판본 md5 `0c49f327`(11줄, 저장소판 표식 없음) → scp·install 후 **`acf96a6f` = 저장소판과 완전 일치**, 권한 755 root. ② 이어서 `stop_server.sh` 전 구간 실행 — 백업 `blog-2026-07-20-0239.sql.gz`(333KB) 생성으로 **버킷 3→4**, 주차 `No changes`(이번엔 오리진을 안 바꿨으니 정상), 정지 완료, 검증 4종 전부 기준선 일치. `install && 실행`이 `&&` 체인이라 산출물이 생겼다는 건 install이 성공했다는 뜻이기도 하다.
- **비용**: $0. git 파일이라 AWS 리소스가 아니다. (이 시점 실측: 이번 달 서비스별 실청구가 전부 1센트의 몇 만분의 1 수준, 백업 3개 합계 967 KiB → 월 $0.00002.)
- **배운 것**: "서버에 심어두면 된다"는 건 **인스턴스가 영원하다는 가정**에 기대는 것이다. 이 프로젝트는 EC2를 껐다 켜고 언젠가 교체할 수도 있으니, 서버 상태에만 사는 물건은 사라질 후보다. cron이 안 돈 것도, 스크립트가 저장소에 없던 것도 뿌리는 같다 — **코드 리뷰에 안 걸리는 곳에 로직을 두면 아무도 못 본다.**

### 🤖 Dependabot PR 7개 정리 — 5개 병합, 2개 반려 (2026-07-20)

`gh`가 없어 PR 화면은 못 봤지만 브랜치를 fetch하면 **내용은 로컬에서 다 보인다.** 7개를 전부 뜯어보고 위험도별로 갈랐다.

**병합한 것과 그래서 뭐가 좋아졌나**

- **terraform aws 5.100 → 6.55** — 얻는 기능은 없다. 실익은 **다음 업그레이드가 작아지는 것**이다. 프로바이더 메이저를 미루면 나중에 5→7 같은 한 방이 되고, 그때는 리소스 재생성 위험을 안고 넘어야 한다. 지금은 `plan`이 `No changes`라 공짜로 건널 수 있는 창이었다.
- **actions/checkout 4 → 7** — 실질 이득이 제일 큰 항목. v6부터 **깃 자격증명을 로컬 git config가 아니라 `$RUNNER_TEMP`에 둔다** → 토큰이 워크스페이스에 남아 후속 스텝·아티팩트로 새는 경로가 줄었다. v7은 `pull_request_target`·`workflow_run`에서 fork PR 체크아웃을 차단한다(전형적인 pwn-request 공격 차단). 지금은 그 트리거를 안 써서 즉시 이득은 없지만, 나중에 추가해도 안전한 기본값이 깔린 셈.
- **setup-node 4 → 7 / setup-python 5 → 6 / configure-aws-credentials 4 → 6** — 셋 다 핵심은 **Node 24 런타임 전환**이다. GitHub이 옛 런타임 액션을 단계적으로 밀어내므로, 안 올리면 언젠가 경고를 거쳐 강제로 끊긴다. 미리 넘어두면 그날 급하게 안 고쳐도 된다. 입력(`node-version: 20`·`python-version: '3.12'`·`role-to-assume`)은 전부 그대로라 코드 수정 0.

**반려한 것**

- **frontend deps(10개) — 병합 불가**: `typescript`를 `~7.0.2`로 올렸는데 `typescript-eslint@8.64.0`의 peer가 `typescript >=4.8.4 <6.1.0`이다. **`npm ci`가 ERESOLVE로 실패** = 락파일이 깨진 PR. Dependabot이 그룹 업데이트하며 호환성을 놓친 케이스. typescript-eslint가 TS 7을 지원하면 자동 재생성되므로 그때 병합하면 된다. **그대로 둔다.**
- **backend deps(23개) — 보류**: `bcrypt 4→5`는 안전(이 코드는 `hashpw`/`checkpw` 직접 호출, breaking change는 passlib 연동 쪽인데 안 씀). 문제는 `openai 1.55→2.46`·`google-genai 0.8→2.12`·`cohere 5.13→7.0` **메이저 3종**이 전부 BYOK 경로라는 것. 이 경로는 테스트에서 목킹되므로 **CI가 초록이어도 실제 호출은 검증되지 않는다**(:1058의 '남은 것'이 정확히 여기). 깨져도 블로그 본체엔 영향 없는 선택 기능이라 급하지 않아, 목킹을 심화하거나 실제 키로 한 번 태워본 뒤에 올리는 게 맞다.

**충돌 처리**: `checkout`과 `setup-node`/`setup-python`이 ci.yml의 **같은 두 줄**을 각자 하나씩만 올려 2건 충돌. 두 의도가 배타적이지 않아 양쪽 다 올린 값으로 합쳤다(둘 다 v7/v6). 검증: 충돌 마커 0, YAML 파싱 통과, 입력값 전부 보존.

**놓칠 뻔한 것**: 병합 후 로컬 `.terraform/`엔 옛 프로바이더(5.x)만 있어서, 이대로면 다음 `stop_server.sh`가 **백업을 마친 뒤 주차 단계에서** `terraform init` 요구로 실패했을 것이다. `terraform init`으로 6.55를 받아두고 `plan No changes` 재확인.

- **라이브 실증**: push 후 CI·Deploy 둘 다 success. 잡 단위로 확인 — `checkout@v7`·`setup-python@v6`·`setup-node@v7`·pytest·eslint·vitest·빌드 전부 실행됨, 그리고 가장 불확실했던 **`configure-aws-credentials@v6`의 OIDC 역할 assume 성공** → S3 업로드·CloudFront 무효화까지 완료. 프론트 홈 200.
- **배운 것**: **"깨지면 티가 난다"와 "안 깨진다"는 다른 말인데 섞어 쓰기 쉽다.** 처음에 Actions 4개를 "무난"이라고 했지만 실제로는 검증한 게 아니라 *복구 가능성*을 말한 것이었다 — 지적을 받고 릴리스 노트를 읽고서야 근거가 생겼고, push 후 잡 로그를 보고서야 사실이 됐다. 추론·검증·실증은 단계가 다르고, 어느 단계인지 말하지 않으면 상대는 제일 센 단계로 알아듣는다.

### 🤖 AI 모델 Sonnet 4.6 → Sonnet 5 + 남용 방어 2건 [완료/라이브검증] (2026-07-20)

**모델 교체 — ID만 바꿨으면 초안이 잘렸다.** Sonnet 4.6은 `thinking`을 생략하면 사고가 꺼진 채 돌았지만, **Sonnet 5는 생략 시 adaptive가 기본**이다. `max_tokens`는 사고+본문 합산 상한이라 2500이면 사고가 예산을 먹고 본문이 truncate된다 — **Fable 5에서 이미 겪은 것과 같은 함정**. Fable 전용이던 분기를 `_THINKING_ON_BY_DEFAULT` 집합으로 일반화하고 Sonnet 5도 `effort=low` + 6000토큰으로 태웠다. (지연이 더 중요하면 `thinking:disabled`로 끌 수 있다 — Sonnet 5는 받지만 Fable 5는 400이라 못 끈다.)
- 바꾼 곳 4군데: `MODELS`, `_CLAUDE_FREE`/`_CLAUDE_ALL`, `config.ai_model`. 프론트는 API에서 목록을 받는 구조라 손댈 게 없었다.
- 비용: 4.6과 같은 $3/$15, 2026-08-31까지 인트로 $2/$10. 다만 **새 토크나이저라 같은 글이 토큰 ~30% 더** 나온다 → 인트로 중엔 오히려 싸고, 끝나면 실질 ~30% 상승.

**남용 방어 ①: 인메모리 레이트리밋 → Postgres 시간당 캡.** slowapi의 `10/hour`는 메모리라 컨테이너 재시작에 0으로 돌아가고 IP 기준이라 IP를 바꾸면 피할 수 있었다. `limits 5.8.0`이 지원하는 영속 저장소는 Redis/Memcached/MongoDB뿐(**설치해서 스킴 목록으로 확인** — SQL 백엔드 없음)이라, t2.micro에 서비스를 늘리는 대신 검증된 `ai_usage` 패턴을 확장했다.
- 새 테이블 `ai_hourly_usage(user_id, hour, count)` + 마이그레이션 `b7f1c4e29a03`.
- 일일 캡과 **의도적으로 다른 점 셋**: ① BYOK도 센다(비용은 사용자 부담이지만 워커 스레드는 우리 자원) ② 성공이 아니라 **'시도'**를 센다(실패를 안 세면 죽은 엔드포인트 무한 재시도가 공짜가 되어 방어가 무의미) ③ DB라 재시작을 견딘다.
- slowapi 리밋은 **그대로 둔다** — 싼 인메모리 1차 필터 + DB 최종 방어선의 2중 구조.

**남용 방어 ②: `compatible` 타임아웃 60s → 20s.** 사용자가 `base_url`을 직접 정하는 유일한 경로라, 일부러 느린 호스트를 걸어 워커 스레드를 60초씩 묶는 게 가능했다. 벤더 엔드포인트는 주소가 고정이라 위협 벡터가 아니고 정상 호출이 길 수 있어 60초 유지.

- **라이브검증(2026-07-20)**: 백엔드 재배포 → alembic `2a2a9af10b7c` → **`b7f1c4e29a03`**, `ai_hourly_usage` 생성 확인. **Sonnet 5 실호출 882자·안 잘림**(끝맺음이 완결된 플레이스홀더 줄, 코드블록 0). `/ai/draft` 1회 → `ai_hourly_usage` 0 → `count=1`. CI 114개 통과(신규 11개).
- **배운 것**: 모델 교체는 **ID 문자열 치환이 아니다.** 같은 요청이 새 모델에서 다르게 해석되는 '조용한 기본값 변화'(여기선 adaptive thinking)가 진짜 위험이고, 그게 `max_tokens` 같은 남의 설정과 만나 터진다. 마이그레이션 가이드를 먼저 읽은 게 truncation 한 번을 아꼈다.

### 🔍 보안 전면 점검 — 11개 영역, 뚫린 곳 없음 (2026-07-20)

"DoS·유해 콘텐츠가 뚫릴 수 있다"는 우려에서 시작해 전 영역을 코드 근거로 훑었다. **결론: 점검한 11개 중 10개가 이미 막혀 있었고, 열려 있던 1개도 보안이 아니라 UX 결함이었다.** 없는 일을 만들기보다 "확인해보니 막혀 있다"가 정확한 답이라 대부분 코드를 안 건드렸다.

- **SQL 인젝션**: 쿼리 문자열 보간 **0건**, 동적 ORDER BY **0건**(11곳 전부 코드 상수), `text()` 8곳은 정적 리터럴 또는 바인딩(`:since`). 검색 `q`의 `f"%{...}%"`는 SQL이 아니라 **LIKE 패턴 '값'**이고 `ilike()`의 바인딩으로 들어간다. `_like_escape`는 역슬래시를 먼저 치환해 이중 이스케이프 버그도 없다.
- **CSRF**: **해당 없음** — 토큰이 쿠키가 아니라 `Authorization: Bearer`. 브라우저가 교차 사이트에서 자동으로 안 붙인다.
- **세션·재설정**: JWT의 `token_version` 스냅샷을 매 요청 대조 → 재설정·차단 시 기존 토큰 즉사. 재설정 토큰도 같은 `ver`를 써서 **1회용**이 된다(메커니즘 재사용이 깔끔).
- **이메일 열거**: 계정 존재 여부와 무관하게 동일한 202.
- **IDOR**: 글은 `visible_condition()` **하나**를 목록·검색·메타가 공유. 댓글은 `_viewable_post_or_404`가 **조회·작성 양쪽**에 걸려 못 보는 글의 댓글은 읽지도 쓰지도 못하고 **404**라 존재 여부도 안 흘린다.
- **업로드**: **매직바이트로만** 판별해 content_type·확장자를 도출(사용자 입력 불신), 파일명 uuid → 경로조작 불가, 5MB를 +1바이트로 초과 감지.
- **결제**: 금액을 서버가 정하고 `/confirm`에서 대조 → 위변조 차단. 멱등이라 중복 승인 안전.
- **시크릿**: `.env` 미추적, 추적 파일 전체 스캔(`sk-ant-`/`AKIA`/`ghp_`/`xox`/PRIVATE KEY) **0건**, 프론트 번들엔 `VITE_API_BASE`뿐.
- **AI SSRF**: `validate_base_url`이 https 강제 + **호스트네임을 실제 IP로 풀어 모든 결과를 `is_global` 검사** → `169.254.169.254`(링크로컬) 거부. BYOK 키는 Fernet 암호화·`user_id` 스코프·응답 미노출.
- **AI 티어 우회**: 카탈로그 모델이면 `body.provider`를 **아예 무시**하고 서버 매핑을 쓴다 → 무료 사용자가 provider를 조작해 서버 키로 Opus를 태울 경로가 없다.
- **레이트리밋 우회**: `client_ip()`가 XFF의 **맨 뒤**(프록시 관측값)를 쓴다. 그 docstring이 "8000을 CloudFront에만 열어야 완전"이라고 남은 구멍을 경고하고 있었는데, **확인해보니 이미 조치돼 있었다** — 8000 인바운드가 AWS 관리 프리픽스 `com.amazonaws.global.cloudfront.origin-facing`으로 제한(0.0.0.0/0 아님), SSH는 단일 IP.

**열려 있던 1개 — `cover_image` 스킴 미검증** (:797에 3주간 "남은 것(낮음)"으로 있던 항목). **XSS는 아니다**: `img src`의 `javascript:`는 현대 브라우저에서 실행되지 않고 React도 막는다. 실제 문제는 **조용한 실패**였다 — CSP가 `img-src 'self' data: https:`라 `http://` 커버는 브라우저가 차단하는데 저장은 성공해서, 사용자는 이미지가 왜 안 뜨는지 알 방법이 없었다. 저장 시점 422로 바꿨다(허용: https·상대경로·`http://localhost`—로컬 `public_base_url`이 http라 막으면 자기 업로더가 거부됨). `//host/x`는 상대경로처럼 보이지만 **외부 호스트**라 거부. 라이브 검증: https 201 / http·프로토콜상대·`javascript:` 전부 422.

**주석이 함정이던 것 — IMDS hop-limit.** `validate_base_url` docstring이 "최종 방어선은 IMDSv2 + **hop-limit 1**"이라 적혀 있었는데 실제는 **2**였다. 값이 아니라 주석이 틀렸고, **2를 1로 낮출 수도 없다** — 백엔드가 컨테이너에서 인스턴스 역할로 S3에 업로드하는데 도커 브리지가 홉을 하나 더 먹어서, 1이면 업로드가 깨진다. 주석을 믿고 "하드닝"하면 **보안은 그대로인데 기능만 부수는** 함정이었다. rebinding의 진짜 방어선은 `http_tokens=required`(IMDSv2)다 — 자격증명을 꺼내려면 PUT으로 토큰을 받아 헤더에 실어야 하는데, 이 SSRF는 '주소만' 조종 가능하고 요청 모양(OpenAI SDK의 POST+JSON)은 못 바꾸므로 핸드셰이크를 완성할 수 없다. `ec2.tf`에도 "1로 낮추지 말 것"과 이유를 남겼다.

- **배운 것**: **보안 점검의 결과가 "고칠 게 없다"인 것도 성과다.** 우려가 들어왔을 때 뭐라도 고쳐서 대응한 척하는 게 제일 나쁘다 — 실제로 뚫렸는지 근거로 확인하고, 안 뚫렸으면 안 뚫렸다고 말하는 게 맞다. 그리고 **주석은 코드보다 빨리 썩는다**: 이번에 실제로 위험했던 건 취약점이 아니라, 없는 방어를 있다고 적어둔 주석이었다.

### 📦 backend deps 23개 병합 — 보류(:1222)를 근거로 해소 [완료/로컬실증] (2026-07-22)

:1222에서 "메이저 3종이 전부 BYOK 경로라 **CI가 초록이어도 실제 호출은 검증되지 않는다**"며 보류했던 건. 그 우려를 그대로 두고 병합하지 않으려고, 테스트가 못 닿는 자리를 따로 확인했다.

- **실증(측정한 것)**: 이미지 재빌드로 23개 **의존성 해결 성공**(충돌 0) → `ruff 0.15.22` 통과(0.8→0.15 점프인데 신규 위반 0) → **pytest 9.1.1로 114개 전부 통과**, 커버리지 79.22%. 그리고 **LLM 클라이언트 5종 생성**(claude·openai·compatible+base_url·gemini·cohere)을 코드가 넘기는 kwargs 그대로 태워 확인 — 07-20에 실제로 겪은 사고(:667 `openai==1.54.3`+`httpx 0.28.1`에서 **생성 자체가** 깨져 BYOK 502)와 같은 모양이라 여기를 짚었다.
- **검증(문서 대조)**: 메이저 3종의 breaking change가 이 호출부와 겹치는지 릴리스 노트로 확인. `cohere 7.0`은 **최소 파이썬 3.8→3.10이 유일한 breaking**(우린 3.12), `google-genai 1.0`은 `deprecated_response_payload` 제거뿐이고 `2.0`은 **"Interactions Only — GenerateContent usage is unaffected"**라고 명시, `openai 2.0`은 Responses API의 tool output 타입 하나뿐이라 `chat.completions`와 무관.
- **아직 실증 아닌 것**: **실제 키로 한 번도 안 태워봤다.** 생성까지는 실측, 그 뒤 `chat`/`generate_content` 응답 파싱은 문서 대조까지다. :1222가 원한 "실제 키로 한 번" 은 여전히 남아 있다 — 다만 깨져도 블로그 본체가 아니라 선택 기능(BYOK)이고, 이제 근거가 '없음'에서 '문서 대조'로 올라왔다.
- **덤으로 드러난 것 — CI 실패가 deps 탓이 아니었다**: 그 PR의 CI가 빨간 건 브랜치 base가 `9de726e`(07-20 작업 **이전**)라서였고, 로컬 첫 실행에서도 30개가 깨졌는데 원인은 **로컬 `blog_test` DB가 `approved`/`notify` 마이그레이션 이전 상태**였다. conftest의 `create_all`은 **없는 테이블만** 만들지 기존 테이블에 **컬럼을 붙이지 않는다** → `column author_subscriptions.approved does not exist`. CI는 매 실행이 새 postgres 서비스 컨테이너라 절대 안 겪는다. DB를 drop하고 재생성하니 114개 전부 통과.
- **남긴 경고**: `starlette 1.3`이 `StarletteDeprecationWarning: Using httpx with starlette.testclient is deprecated; install httpx2 instead`를 띄운다. 지금은 경고일 뿐이지만 다음 메이저에서 TestClient가 깨질 자리 → 그때 `httpx2`로 옮긴다.
- **배운 것**: **"CI가 빨갛다"를 "이 변경이 깨졌다"로 읽으면 안 된다.** 이번 빨간불은 오래된 base + 오래된 로컬 DB 두 가지였고 deps는 무죄였다 — 실패를 봤을 때 제일 먼저 물을 건 "무엇이 실패했나"가 아니라 **"무엇과 무엇을 비교한 결과인가"**다. 그리고 **로컬 테스트 DB는 스키마가 낡아도 조용하다**: CI는 매번 새 DB라 이 어긋남을 영원히 안 잡아주고, 로컬에서만 터지므로 "내 환경 문제"로 넘기기 쉽다.

### 🔴 bcrypt 5.0 회귀 — 위 병합이 만든 500을 배포 전에 잡음 (2026-07-22)

바로 위 절에서 "병합해도 된다"고 결론 낸 뒤 코드검사를 돌렸더니 **내가 만든 회귀**가 나왔다. 배포 전에 잡아서 라이브 사고는 없었다.

- **증상**: 72**바이트**를 넘는 비밀번호로 가입·로그인하면 **HTTP 500**. `bcrypt 5.0`이 초과 입력을 `ValueError`로 거부한다(`4.2.0`은 조용히 잘랐다 — 옛 버전을 설치해 양쪽을 직접 재서 확인). 더 나쁜 쪽은 가입이 아니라 **로그인**이다. `verify_password`도 같이 죽어서 **기존 사용자가 500으로 잠긴다** — 401이 아니라 500이라 본인은 이유도 알 수 없다.
- **뿌리는 단위 불일치**: `schemas/user.py`의 `PW_MAX = 72`는 Pydantic `max_length`라 **글자 수**를 세는데 bcrypt의 72는 **바이트**다. 한글은 글자당 3바이트라 **24글자만 넘으면** 스키마는 통과하고 bcrypt에서 터진다. 영문만 쓰면 평생 안 만나는 버그다.
- **조치**: `_bcrypt_input()`으로 `encode()[:72]` — `4.x`와 **정확히 같은 동작**을 복원. 저장된 해시가 이미 '잘린 72바이트'로 만들어진 것이라 같은 방식으로 잘라야 검증이 맞는다. 스키마에서 422로 거부하는 안도 있었지만 그러면 긴 비번을 쓰던 기존 사용자가 재설정 전까지 못 들어온다.
- **진짜 수정은 테스트**: 기존 114개가 못 잡은 이유는 **전부 짧은 ASCII 비밀번호**를 써서다. 경계값 테스트 2개(90바이트 한글 가입·로그인)를 추가하고, **수정을 되돌려 빨개지는지 확인한 뒤** 넣었다(ruff 보안규칙 켤 때와 같은 방식). 116개 통과.
- **배운 것**: **위험하다고 표시한 것은 다 무사했고, 안전하다고 넘긴 것이 터졌다.** :1222는 메이저 3종(openai·genai·cohere)을 위험으로 지목하고 `bcrypt 4→5`는 "직접 호출이라 안전"이라고 **추론으로** 넘겼다. 그 추론은 passlib 얘기로는 맞았지만 **72바이트 처리 변경**을 몰랐다 — 위험 목록을 만들 때 '무엇을 조사할지'가 아니라 '무엇을 조사 안 해도 되는지'를 정하는 쪽이 더 위험하다. 그리고 **한글이 기본 언어인 서비스에서 '글자 수'와 '바이트'를 섞어 쓰면 영문 테스트로는 영원히 안 걸린다.**

### 🚀 deps 배포 + 개발일지 #17 발행 [완료/라이브검증] (2026-07-22)

위 두 절(deps 병합·bcrypt 회귀 수정)을 프로덕션에 실제로 올린 세션. **켜기 → 배포 → 발행 → 끄기**.

- **켜기**: EC2 start → 새 주소 `ec2-3-35-21-2` → `terraform apply -var=backend_origin_dns=...`(1 changed).
- **배포 전 기준선을 먼저 세움**: 유저 5·글 26·구독 6(승인 6/6)·댓글 4·알림 0·연재 16·alembic `b7f1c4e29a03`. 07-20에 배운 대로 대조값 없이 배포하지 않는다.
- **이번엔 마이그레이션이 없다**: 로컬·프로드 head가 둘 다 `b7f1c4e29a03`이라 순수 코드+deps 교체. 배포 후에도 head 동일 = 스키마 무변경 확인.
- **배포**: tar(app/alembic/alembic.ini/requirements/Dockerfile) → scp → 추출. **`.env` md5를 추출 전후로 대조**해 보존 확인(`3edd4498…` 동일). 재빌드는 규칙7대로 사용자가 실행 → 23개 설치·이미지 재생성·컨테이너 교체 성공.
- **핵심 검증 — bcrypt 회귀가 실제로 고쳐졌는가**: 프로드 컨테이너에서 90바이트 한글 `hash_password`/`verify_password` 왕복 성공(맞는 값 True·틀린 값 False·ASCII 72자 True). **가입 API로 테스트 계정을 만들지 않았다** — 프로드 데이터를 오염시키지 않으려고 함수를 직접 태웠다. 기존 유저 5명 해시도 전부 `$2b$`로 **예외 없이** 판정됨(= 잠기는 사람 없음).
- **데이터 무결성**: 배포 후 유저 5·글 26·구독 6(승인 6/6)·댓글 4·알림 0 — 기준선과 완전히 동일.
- **발행**: `#17` 생성 1건. **연재 16 → 17편**. 소급 날짜(KST 09:00 → UTC 00:00)·public·태그 5개·본문 6,313자·`owner_id=3`·`series="블로그 만들기"` 확인. 라이브 `/api/posts/29/series`가 **`total:17, index:17`** — 07-20에 null이었던 연재 네비가 이번엔 스크립트가 채웠다(그때 넣은 `series` 설정이 실제로 동작).
- **걸린 것 — `PYTHONPATH`**: `/tmp`의 스크립트를 `python /tmp/x.py`로 돌리면 `sys.path[0]`이 `/tmp`라 `app` 모듈을 못 찾는다(`ModuleNotFoundError`). `-e PYTHONPATH=/app`이 필요하다. `publish_devlogs.py` docstring의 실행 예시엔 이게 빠져 있어서 다음에도 걸린다 → 고쳐둘 것.
- **끄기**: `stop_server.sh` 본 경로 — 백업 산출물 `blog-2026-07-22-0230.sql.gz`(339KB, 버킷 4→5) → 주차(1 changed) → 정지 → 검증 4종(stopped·PublicIp null / EIP 0 / 프론트 200 / `/api/status` 504) 전부 기준선 일치.

### 🛟 백업·복원 전면 보강 — 훈련이 통과시키던 13가지 [완료/라이브적용] (2026-07-22)

07-22에 `restore_drill.sh`의 `head -1` 버그(b754c4c)를 고친 뒤 "이게 최선이냐"는 물음으로 다시 훑었다. 훈련 **안쪽** 6가지보다 **바깥쪽**(백업 전략 자체) 7가지가 더 컸다.

- **전제부터 틀렸던 것 — 업로드 이미지**: 처음엔 "이미지가 EC2 디스크에만 있어 EBS 유실 = 전멸"로 봤는데, 코드를 보니 **2026-06-26에 이미 S3로 옮겼다**(:523). 인스턴스 교체엔 안전하다. 진짜 위험은 다른 데 있었다: 이미지가 **프론트와 같은 버킷**에 살고 배포가 `s3 sync --delete`라, 지금은 deploy.yml의 `--exclude "uploads/*"` **한 줄**이 유일한 방어선이고 버저닝이 없어 되돌릴 수도 없다. → 조사를 안 했으면 '있지도 않은 구멍'을 막는 코드를 쓸 뻔했다.
- **훈련의 판정 기준을 다시 세움**: 예전엔 행 수 불일치를 FAIL로 처리해서, 훈련이 초록불이려면 **백업 이후 아무도 글을 쓰지 않아야** 했다. 이제 ① **구조 검사**(테이블 집합·인덱스·제약·시퀀스·alembic 버전·확장·쓰기 가능·BYOK 복호화)만 합격/불합격을 가르고, ② **행 수**는 방향으로 분류해(`DRIFT` = 백업 이후 쓰기 / `CHECK` = 복원본이 더 많음) **백업에 안 담긴 구간의 실제 시각**을 보여준다.
- **하드코딩 제거**: 테이블 5개 → `information_schema`로 **12개 전수**, 시퀀스 3개 → nextval 기본값을 가진 컬럼을 찾아 **11개 전수**. 빠져 있던 `payments`·`llm_credentials`·`ai_usage` 등 잃으면 제일 아픈 것들이 그동안 훈련 밖에 있었다.
- **잃어도 안 보이던 손실 2종을 검사에 넣음**: (1) **alembic_version 대조** — 다르면 행 수가 다 맞아도 앱이 그 DB 위에서 안 뜬다. (2) **BYOK 카나리아** — 복원된 암호문을 현재 `LLM_ENCRYPTION_KEY`로 실제로 복호화해 본다. 이 키는 `.env`에만 있고 어떤 백업에도 없어서, 잃으면 `llm_credentials`가 완벽히 복원돼도 **영원히 못 푼다**. 행을 세는 검사로는 원리적으로 안 보인다.
- **DB 바깥까지 봄**: 복원된 글이 참조하는 `/uploads/*` 파일명을 뽑아 **S3에 실제로 있는지** 대조(없으면 복원해도 이미지가 깨진다), 이미지 미러 개수, 만료 안 되는 `keep/latest.sql.gz` 존재, `.env` 사본 유무까지.
- **훈련이 뒷정리를 안 하던 것**: `trap`으로 **무조건** `restore_test`를 drop(07-22에 실제로 남았다). 복원 실패 시 `/tmp/restore.log` **끝 30줄을 보여준다** — 예전엔 로그를 남기기만 하고 정작 실패하면 아무것도 안 보여준 채 죽었다.
- **잘린 덤프**: `pg_dump`가 중간에 죽어도 gzip은 받은 데까지 정상 종료해 `gzip -t`가 통과한다. → 백업 스크립트를 스트리밍에서 **'파일로 떨구고 검사 후 업로드'**로 바꿔 완료 표식(`PostgreSQL database dump complete`)·최소 크기를 확인하고, 정지 절차는 **직전 백업 대비 크기 급감**(절반 미만)이면 정지를 막는다. 산출물 확인도 목록 줄 수 세기 → **키 이름으로 `head-object`**.
- **백업이 사이트 라이브 중에 떠지던 것**: 정지 절차를 **백업→주차에서 주차→백업으로** 뒤집었다. 예전 순서는 pg_dump 스냅샷 이후 주차까지 몇 분간 들어온 글·댓글·결제가 사본에 없는 채로 서버가 꺼졌다.
- **인프라(terraform)**: 백업 버킷과 프론트 버킷 **버저닝** 활성(EC2가 탈취돼도 PutObject로 기존 백업을 **덮어써 파괴**할 수 있었다 / `sync --delete` 사고를 되돌릴 수 있게), 만료를 `blog-` 접두사 **180일**로 좁히고 `keep/`·`uploads/`는 **만료 대상에서 제외**(백업이 '끌 때만' 도는 구조라 한 달 넘게 서버를 안 켜면 옛 30일 설정에선 **백업이 0개**가 되는 구간이 있었다 — cron이 한 번도 안 돌던 것과 같은 '조용한 소멸'), EC2 IAM을 `blog-*`로 축소해 `keep/`·`uploads/`는 아예 못 건드리게.
- **새 스크립트/문서**: `scripts/env_escrow.sh`(운영 `.env` 사본 보관·해시 대조. 값은 절대 출력 안 하고, 키 교체 시 **옛 사본을 보존**한다 — 옛 암호문은 옛 키로만 풀린다), `RECOVERY.md`(재해 복구 런북 4종. **인스턴스 terminate = DB 소멸**이라는 점과, 복원 시 **백엔드를 먼저 띄우면 alembic이 빈 스키마를 만들어 덤프와 충돌**한다는 순서 함정을 적었다).
- **검증(실제로 돌린 것)**: 로컬 dev Postgres에 훈련 로직을 그대로 태워 **테이블 12개·시퀀스 11개·인덱스 36·제약 88·확장 2종 전부 통과**, `status_checks`가 `DRIFT 운영 1362 / 복원 1360` + **미백업 구간 시작 시각**까지 출력되는 것 확인. 고의로 깨진 덤프로 실패 경로도 확인 — **로그 표시 → `restore_test` 자동 삭제 → 비정상 종료** 순으로 동작. `terraform plan`은 **3 add / 2 change / 0 destroy**(CloudFront·EC2 무영향).
- **규칙7을 내가 과하게 적용했던 것**: 처음엔 `terraform apply`까지 "사용자가 실행"으로 넘겼는데, 규칙7의 경계는 **'프로덕션 앱 코드를 갈아끼우는 명령'**(`up -d --build`)이지 `terraform apply`·백업 실행·DB 조회가 아니다. 사용자가 지적해 바로잡고 직접 실행했다. **안전 규칙을 넓게 해석하는 것도 비용이다** — 할 수 있는 일을 안 하고 넘기면 구멍이 열린 채로 남는다.
- **라이브 적용(같은 세션)**: `terraform apply` → **3 added / 2 changed / 0 destroyed**. AWS 실제 상태로 재확인 — 두 버킷 버저닝 `Enabled`, 백업 lifecycle(`blog-` 180일 / noncurrent 90일 / MPU 중단 7일), EC2 IAM이 `.../blog-*`로 축소됨. `env_escrow.sh save`로 운영 `.env` 15개 항목(`LLM_ENCRYPTION_KEY` 포함) 보관(권한 600/700, 서버 해시와 일치). **13가지 중 열려 있던 마지막 실제 위험이 닫혔다.**
- **런북의 가장 위험한 주장을 실증**: `RECOVERY.md` 시나리오 B의 복원 순서는 추론으로 쓴 것이라 **실제 운영 백업으로 재현**했다. 빈 DB에 복원 → 글 27·유저 5 복원되고 `alembic current`가 `b7f1c4e29a03 (head)`·`upgrade head`는 no-op(= 순서가 맞다). 반대로 백엔드를 먼저 띄우면 `ERROR: relation "ai_hourly_usage" already exists`로 **복원이 중단되고 글 0건 빈 DB가 남는다**. 사고 한복판에서 이걸 만나면 "복원했는데 글이 없다"로 보여 백업을 의심하게 되는데, 백업은 멀쩡하고 순서가 틀린 것이다.
- **아직 아닌 것**: 시나리오 B의 **인스턴스 재생성·맨바닥 부트스트랩**(docker 설치·코드 업로드·`.env` 배치)은 운영 인스턴스를 terminate해야 밟히는 경로라 일부러 안 했다. 임시 인스턴스로 한 번 끝까지 해보는 게 다음 할 일.

### 🔥 재해복구 리허설 — 런북대로 맨바닥에서 재건해보니 런북이 틀렸다 (2026-07-22)

`RECOVERY.md` 시나리오 B(인스턴스 유실)는 "재료가 다 있는지 확인해 쓴 절차이지 실증이 아니다"라고 적어둔 상태였다. 운영을 건드리지 않고 **임시 인스턴스(t2.micro + 전용 보안그룹, SSH는 내 IP만)를 띄워 처음부터 끝까지 밟아봤다.** terraform state를 안 건드리려고 CLI로 만들고, 끝나고 terminate + SG 삭제까지 확인했다(남은 인스턴스 1대·볼륨 1개 = 운영뿐).

- **결과**: 맨바닥 AL2023에서 docker 설치 → 코드 업로드 → 일회용 `.env` → **DB만 먼저** → 실제 운영 백업 복원(테이블 0개 → **글 27·유저 5·테이블 12·alembic `b7f1c4e29a03`**) → 백엔드 빌드·기동 → `/api/status` 200, `/api/posts` 200으로 **복원된 글이 API로 그대로 나오는 것**까지 확인. 기동 로그에 `Running upgrade`가 없다 = alembic이 no-op(순서가 맞다).
- **찾은 버그 ①(빌드 자체가 안 됨)**: AMI에 딸려오는 buildx가 0.12.1인데 최신 compose가 `compose build requires buildx 0.17.0 or later`로 거부한다. 런북 2단계가 compose만 깔고 buildx는 안 깔았다. 운영과 같은 **v0.35.0**을 설치하는 줄을 추가했다. (운영·임시 둘 다 조회해 대조 — 추측 대신 운영이 실제로 쓰는 조합에 맞췄다)
- **찾은 버그 ②(더 나쁨 — 시크릿이 이미지에 구워짐)**: 3단계 tar에 **`.dockerignore`가 빠져 있었다.** 그러면 `COPY . .`가 `.env`를 이미지 안에 굽는다(파일 주석이 "시크릿은 절대 이미지에 굽지 않음"이라고 못박아둔 바로 그 일). 게다가 앱이 안 뜬다 — pydantic이 dotenv의 여분 키(`DB_PASSWORD`·`ADMIN_EMAIL`, `Settings`에 없는 필드)를 `extra_forbidden`으로 거부해 **백엔드가 재시작 루프**에 빠졌다. `.dockerignore`를 넣어 재빌드하니 `/app/.env` 없음 + 정상 기동.
- **왜 운영에선 안 터지나 = 왜 못 찾았나**: 운영 `~/blog`엔 예전 배포 때 올라간 `.dockerignore`가 **이미 있다**. 그래서 평소 배포는 그 파일을 안 보내도 멀쩡하고, **새 인스턴스에서만** 터진다. 코드를 아무리 읽어도 안 보이고 서버 상태를 대조해야 보이는 종류였다.
- **배운 것**: **런북은 밟아보기 전까진 문서가 아니라 가설이다.** 두 버그 다 "필요한 재료가 있는가"만 확인해선 절대 안 나오고, 실제로 빈 서버에서 명령을 순서대로 쳐야 나온다. 그리고 하필 **재해 한복판**에서 처음 만나면 — 빌드가 안 되고, 고쳐도 앱이 재시작 루프를 돌고, 원인은 pydantic 검증 오류로 보인다 — 백업이나 덤프를 의심하며 시간을 태웠을 것이다.
- **비용**: 임시 인스턴스 1시간 미만(수십 원). 운영 DB·오리진·terraform state 무영향.

### 🔎 코드검사 — 내가 만든 치명 버그 1개 + 여태 몰랐던 라이브 장애 1개 (2026-07-22)

위 세 절(백업 보강·적용·리허설)을 독립적으로 코드검사했다. **자기 코드를 자기가 읽는 건 한계가 있어서** 별도 리뷰를 하나 돌리고, 동시에 **아직 한 번도 통째로 안 돌려본 스크립트들을 실제로 실행**했다. 둘 다에서 나왔다.

- **치명 — BYOK 카나리아가 "키 없음"을 "통과"로 찍고 있었다.** `case "$res" in *OK*)` 가 먼저 오는데 **`NOKEY`가 `*OK*`에 매치된다**(N-`OK`-EY). 하필 이 카나리아가 존재하는 유일한 이유가 "키를 잃으면 행 수로는 안 보인다"인데, **그 경우만 정확히 못 잡았다.** 판정 토큰을 서로의 부분문자열이 아닌 `RESULT=DECRYPTED`/`NO_KEY`/`BAD_KEY`로 교체. **문자열 부분매치로 상태를 판정하면 언젠가 이렇게 된다.**
- **라이브 장애 — 이미지 업로드가 AccessDenied로 깨져 있었다(내 변경 이전부터).** 리뷰가 "terraform 어디에도 이미지 업로드 권한이 없다"고 지적해 운영 컨테이너에서 직접 태워보니 진짜 실패했다. 원인: `uploads.py`는 인스턴스 역할로 S3에 올리는데, 그 권한은 별도 역할 `blog-ec2-role`에 CLI로 만들어져 있었고(:525에 "Terraform 미관리 = 드리프트"라고 스스로 적어둔 그것), 백업용 프로파일 `blog-backend`를 terraform이 붙이면서 **교체됐다**(EC2는 프로파일을 하나만 가진다). → `blogplafromops/uploads/*` PutObject를 terraform에 추가·적용, 업로드 성공 확인. 범위는 `uploads/`뿐이라 프론트 번들 덮어쓰기는 여전히 차단됨(직접 확인). **드리프트를 "나중에 정리"로 미루면 다음 변경이 그걸 조용히 부순다.**
- **stop_server.sh — 실패 안내가 통째로 안 나오는 경로**: `if ! scp … || ! ssh … > backup.out` 에서 scp가 실패하면 `||`가 단락돼 ssh가 안 돌고, 그러면 `backup.out`이 없어 다음 줄 `cat`이 죽는다 → `set -e`로 종료. **"❌ 백업 실패"도 재시도 안내도 주차 해제 안내도 한 줄이 안 나온다.** scp 실패는 흔한데(막 켠 인스턴스, sshd 준비 전) 사용자는 `cat: No such file` 한 줄만 본다. → 파일을 미리 만들어 두고, 주차 이후의 **모든** 실패 경로에서 안내가 나오도록 종료 훅으로 옮겼다(예전엔 백업 분기에만 있었다).
- **stop_server.sh — `terraform apply -auto-approve`가 인스턴스를 교체할 수 있었다.** 원한 건 오리진 주차뿐인데 그 시점 플랜 전체를 무승인 적용한다. `ec2.tf`에 replace를 유발하는 변경이 섞이면 루트 볼륨(`pgdata`)째 날아가고, 하필 **백업보다 먼저** 도는 단계다. → 플랜을 먼저 뽑아 `aws_instance` 변경이 있으면 중단하도록. (감지 로직은 합성 플랜으로 검증)
- **런북 버그 2개 더**: 시나리오 A는 1단계 훈련이 지운 `/tmp/restore.sql.gz`를 2단계가 쓴다(다시 올리는 단계 추가). 시나리오 B는 인스턴스를 새로 만들어 놓고 **옛 인스턴스 ID로 조회**한다 → 태그로 찾고, 스크립트 4곳의 하드코딩 ID를 바꾸라는 경고 추가.
- **영구 빨간불이 될 뻔한 것**: 참조 이미지 누락을 FAIL로 처리했더니 훈련이 종료코드 1로 끝났다. 원인은 옛 테스트 글(`ppap`·`123`)이 **06-26 S3 이전 전에 날아간 이미지**를 참조하는 것(:520에 "복구 불가"로 기록된 그 손실, EC2 로컬 디스크도 비어 있음). 백업 결함이 아니므로 WARN으로 내렸다 — **헤더가 약속한 "구조 검사만 합격/불합격"과 코드가 어긋나 있기도 했고, 영구 빨간불은 아무도 안 보는 신호와 같다.**
- **그 밖**: `comm`에 `LC_ALL=C` 누락(파일만 C로 정렬하고 comm은 호출자 로케일), 원격이 죽어 이미지 목록이 안 나왔는데 "참조된 이미지가 없습니다"라는 **초록 문구**를 찍던 것, 시퀀스 요약이 FAIL 없이도 "위 FAIL 참고"라고 하던 것, `--skip_backup` 오타를 조용히 무시하던 것, `pending` 상태에서 주차해 방금 켠 서버를 내리던 것, 버저닝 때문에 실보관이 180일이 아니라 **최장 270일**인 것(주석 정정), `keep/`의 옛 버전이 90일에 사라질 수 있던 것(`newer_noncurrent_versions = 3`) 전부 수정.
- **실제로 돌려본 것**: 운영 대상 복원 훈련 3회(구조 전부 통과, `restore_test` 매번 정리됨), 없는 키로 훈련(깨끗하게 실패·잔재 없음), `blog-db-backup.sh`를 운영에서 실행(**좁힌 IAM으로도 업로드 성공** — 이게 안 됐으면 다음 종료 때 백업이 조용히 실패했다), stop_server의 크기 급감 판정을 경계값으로 검증.
- **배운 것**: 이번에 나온 치명 버그 둘 다 **읽어서는 안 나오고 돌려야 나왔다.** 그리고 하나는 내가 **어제 새로 만든 것**, 하나는 **한참 전부터 조용히 깨져 있던 것**이다 — 새 코드만 의심하면 후자를 영원히 못 본다.

### 🛡 조사에서 나온 것 전부 조치 — 감시 신설·SES 재신청 준비·CI 사각지대 (2026-07-22)

"이게 최선이야?"로 세 갈래(AWS 인벤토리·운영 라이브·저장소/CI)를 동시에 훑은 결과를 순서대로 다 고쳤다. **가장 큰 게 백업이 아니라 제품 쪽에 있었다.**

- **블로그가 4주째 새 사용자를 못 받고 있었다**: SES 프로덕션 액세스가 **거부**(`ReviewDetails.Status = DENIED`, CaseId `178238423300607`)됐는데 아무도 몰랐다. PROGRESS:476에 "심사 대기(~24h)"로 적힌 뒤 확인이 없었다. 샌드박스라 검증된 3개 주소 외엔 발송이 안 되고, 인증메일이 `BackgroundTasks`라 거부 예외가 응답을 못 막아 **가입자는 200을 받고 메일은 안 오고 24시간 뒤 `cleanup.py`가 계정을 지운다.** DB가 증명한다 — SES 연동 이후 가입 완료된 계정 3개의 메일이 검증된 3개 주소와 정확히 일치.
  - `sesv2 put-account-details`는 심사 이력이 있으면 `ConflictException`, Support API는 유료 플랜 전용 → **콘솔에서 사람이** 해야 한다. 실제 구현을 확인해 사실만으로 쓴 사유서(2,575자, 이전 제출은 245자)를 `docs/ses-production-access.md`에 넣어뒀다.
- **그걸 감시해야 할 장치가 오히려 덮었다**: `status.py`의 `mail_ok`가 SMTP 포트에 **TCP 연결만** 해봐서 그 4주 동안 **25,826번 "메일 정상"**이라고 답했다. → `email.py`와 같은 STARTTLS+AUTH까지 하도록 교체. 다만 **이걸로도 샌드박스는 못 잡는다**(로그인은 성공한다) — 그건 계정 설정이라 바깥에서 봐야 한다.
- **감시가 없다는 게 이 프로젝트 모든 사고의 공통 원인이었다**: 계정에 CloudWatch 알람·SNS·CloudTrail·로그그룹이 **전부 0개**, 살아 있는 감시는 월 예산 알림 하나뿐이었다. 07-20 백업 0건도, 07-22 이미지 AccessDenied도, SES 거부도 전부 "사람이 우연히 발견할 때까지 몰랐다"를 공유한다. → **`scripts/watch.sh` + 매시 도는 `watch.yml`** 신설(공개 저장소라 Actions 무료). **AWS 바깥에서** 찌른다 — 앱 자가점검은 `backend_ok`가 하드코딩 `True`(27,506번 중 실패 0건)이고 CloudFront·오리진을 아예 못 본다. 첫 실행에서 실제 문제 2건(API 504, SES 샌드박스)을 그대로 잡았다.
- **결제 안전밸브가 꺼져 있었다**: 운영 `.env`에 `TOSS_SECRET_KEY`도 `PAYMENTS_REQUIRE_LIVE`도 없어 코드 기본값(**토스 공개 문서용 테스트키**)으로 돌았고 `_guard_live()`가 아무것도 안 막았다. → `.env`에 `PAYMENTS_REQUIRE_LIVE=true` 추가(백업본·에스크로 갱신). 재생성 시점에 반영 = 규칙7대로 사용자 실행.
- **권한 잔해 제거**: 휴면 사용자 `github-actions-deploy`(키 0개인데 `blogplafromops` 전체 **DeleteObject** 보유 — 키 하나만 발급되면 DB 덤프에 없는 이미지를 전부 지울 수 있었다), 주인 없는 `blog-ec2-role`/`blog-ec2-profile`. 정책 `github-brench`는 OIDC 역할이 쓰므로 남기고 사용자에서만 분리. 삭제 후 이미지 업로드 회귀 없음 확인.
- **내가 어제 만든 것 중 하나가 같은 병이었다**: 프론트 버킷 버저닝의 정리 규칙을 `prefix = "assets/"`로 걸었는데 `vite.config.ts`가 `assetsDir: ''`라 번들이 **최상위에 평평하게** 떨어져서 **매치하는 객체가 0개**였다. 접두사 추측 대신 `newer_noncurrent_versions = 3`으로 바꿨다 — SPA 번들은 버전이 쌓여 정리되고, uuid 이름이라 버전이 1개뿐인 `uploads/` 이미지는 조건에 안 걸려 **지워진 것도 영구 보존**된다. 그 밖에 정지 절차의 부수 단계 실패가 EC2 정지를 막아 과금이 계속되던 것, `restore_drill.sh`의 `ssh -n` 누락, 시퀀스 목록을 복원본에서만 뽑아 '통째로 없는' 덤프를 통과시키던 것도 수정.
- **문서 10곳이 거짓말하고 있었다**: README의 "일일 pg_dump"·"일일 S3 백업"(그 cron은 한 번도 안 돌아 07-20에 제거됐다), `/uploads/*`가 EC2로 간다는 서술 **4벌**(`main.py`·`uploads.py`·`.gitignore`·`docker-compose.prod.yml` — 오늘 아침 내가 그 주석을 믿고 오판했던 그것), `ec2.tf` "RDS에 연결", `publish_devlogs.py` "RDS는…", 삭제된 `rds-autostop.tf`를 가리키던 `provider.tf`, 낡은 CVE 주석, RECOVERY 자산표의 tfstate 버킷 누락.
- **CI 사각지대를 메웠다**: 셸 스크립트 검증(`bash -n` + shellcheck — 오늘 나온 버그 대부분이 여기였는데 여태 아무도 안 봤다), terraform `fmt`/`validate`, 그리고 **마이그레이션이 빈 DB에서 실제로 올라가는지** + `alembic check`(모델↔마이그레이션 드리프트). 테스트가 `create_all`을 쓰므로 alembic이 깨져도 CI는 초록이었고 프로드 기동 때 처음 드러났다.
- **배포를 코드로 굳혔다**: `scripts/deploy_backend.sh`(`.dockerignore` 포함, `.env` 지문 앞뒤 대조, 재빌드는 규칙7대로 사용자). 절차가 산문으로만 있어서 `.dockerignore` 수정이 RECOVERY.md에만 반영되고 PROGRESS 서술 2곳은 그대로였다. 그리고 `deploy.yml`에 **백엔드 동시 변경 게이트** — 같은 push에 백엔드 코드가 있으면 프론트 배포를 막는다(07-11에 겪은 "프론트가 먼저 나가 API 404"). 프론트 Dockerfile의 Node 22를 CI·배포와 같은 20으로 통일.
- **배운 것**: **감시는 기능이 아니라 전제다.** 오늘 찾은 것 대부분은 몇 주씩 방치돼 있었고, 코드가 틀려서가 아니라 **틀렸다고 말해주는 것이 없어서** 방치됐다. 그리고 나 역시 그 병에 걸린 코드를 하루 만에 만들었다(죽은 lifecycle 규칙) — "설정했다"와 "동작한다"를 가르는 건 의도가 아니라 **확인**이다.

### 🚢 백엔드 배포 — 결제 밸브·메일 점검 반영, 그리고 이미지에 구워져 있던 시크릿 (2026-07-22)

`scripts/deploy_backend.sh`의 첫 실전 실행. **배포 중에 새 문제를 하나 찾아 같이 고쳤다.**

- **기준선 먼저**: 글 27 / 유저 5 / 댓글 4 / 구독 6 / 결제 2 / alembic `b7f1c4e29a03`. 이번 배포에 마이그레이션 변경 없음(순수 코드).
- **찾은 것 — 운영 이미지 안에 시크릿 사본 4개가 구워져 있었다**: `.dockerignore`가 `.env`(정확히 일치)와 `*.env`('.env로 끝나는 것')만 걸러서, **`.env.bak-premigration`·`.env.bak.1782381398`·`.env.save`·`.env.example`이 그대로 `COPY . .`에 실려 있었다.** 그 파일 주석은 "시크릿은 절대 이미지에 굽지 않음"이라고 못박아 두었는데 패턴이 의도를 못 따라간 것이다. 서버에서 `.env`를 손으로 고치면 이런 사본이 자연히 생기므로 사람 주의로는 못 막는다. → `.dockerignore`에 `.env.*` 추가, 백업 사본은 **지우지 않고** 빌드 컨텍스트 밖(`~/env-archive`, 700/600)으로 옮겼다(옛 키가 들어 있을 수 있어서). 재빌드 후 이미지에 env 파일 0개 확인.
- **확인한 것 — 공개 저장소의 `.env.example`은 유출이 아니다**: `SECRET_KEY`에 값이 채워져 있어 놀랐지만 23자 `change-me-in-production` 자리표시자였고, 운영 키와 해시가 다르다(`92809ab6…` vs `7e10697e…`). 값을 출력하지 않고 대조했다.
- **배포 결과**: 재빌드 44.8초, `RestartCount=0`, alembic head 동일, **데이터가 기준선과 완전히 일치**(글 27/유저 5/댓글 4/구독 6/결제 2). `/api/status`·`/api/posts` 둘 다 200.
- **이번 배포로 실제로 바뀐 것 2가지**:
  - `payments_require_live = True` — `_guard_live()`를 직접 태워 **503으로 차단**되는 것 확인. 테스트키로 공짜 Pro가 나가는 경로가 닫혔다.
  - `mail_ok`가 STARTTLS+AUTH까지 하는 검사로 교체 — 운영에서 `True` 확인(발송은 안 함). 이제 자격증명 만료·TLS 오류가 잡힌다. 다만 **SES 샌드박스는 여전히 못 잡는다**(로그인은 성공하니까) → `watch.sh`가 본다.
- **배운 것**: `.dockerignore`의 `.env`/`*.env`처럼 **패턴이 의도를 다 덮지 못하는 경우**가 있다. 주석은 "절대 안 굽는다"고 단언하는데 실제로는 4개가 구워져 있었고, 그 차이는 **컨테이너 안을 들여다봐야만** 보였다. 오늘 하루 반복된 교훈이 여기서도 그대로다 — 선언이 아니라 확인.

### 🩺 컨테이너 헬스체크 · 마지막 IAM 드리프트 회수 · 운영 env 템플릿 (2026-07-22)

ROADMAP을 기준으로 남은 것 중 2시간 안에 끝나는 걸 골라 처리했다. 로드맵의 방학 항목 중 **블로그 마무리·복원 훈련·알림 셋이 오늘 다 끝났고**(체크 반영), 남은 건 ECS(2~3주)뿐이다.

- **backend 컨테이너에 헬스체크 추가**: `db`에는 있는데 `backend`에는 없어서, 프로세스는 살아 있고 응답만 못 하는 상태를 `docker compose ps`로도 알 수 없었다. `python:3.12-slim`에 curl이 없어 표준 라이브러리로 `/api/health`를 때린다(`/health`는 404다 — 그걸 썼으면 100% 실패했다). `start_period=60s`는 기동 때 `alembic upgrade`가 먼저 도는 구간을 실패로 세지 않으려는 것. 배포 후 `Status=healthy, FailingStreak=0` 확인.
  - **오해 금지로 주석에 박아뒀다**: 이건 가시성이지 자동 복구가 아니다. plain Compose는 unhealthy를 이유로 재시작하지 않는다(그건 Swarm/ECS의 몫). 실제 감지는 `watch.sh`가 바깥에서 하고, **이 정의는 ECS로 옮길 때 그대로 태스크 헬스체크가 된다** — 로드맵이 ECS의 난코스로 지목한 바로 그 부분을 미리 맞춰둔 셈이다.
- **`github-brench` 정책을 terraform으로 회수**: 콘솔에서 만들어져 **terraform 밖**에 있던 리소스(→ 2026-07-22 코드검사에서 **"마지막"이 거짓임이 드러났다.** 아래 정정 절 참고). 즉 프론트 배포가 무슨 권한으로 도는지가 저장소 어디에도 없었다. 오늘 사고(`blog-ec2-role`이 조용히 교체돼 이미지 업로드가 AccessDenied)와 **정확히 같은 클래스**라 남겨둘 이유가 없었다. `terraform import` 후 **plan 무변경** — 정의가 실제 정책과 한 글자도 다르지 않다는 증거다(권한을 건드리지 않고 관리만 가져왔다). 관리 리소스 22 → 24개.
  - 회수하면서 알게 된 것: 이 정책은 `blogplafromops` **버킷 전체에 DeleteObject**를 준다. 그 버킷엔 업로드 이미지도 같이 살고 그건 DB 덤프에 안 들어간다 → 배포의 `--exclude "uploads/*"`가 왜 중요한지가 이제 코드 옆 주석으로 남았다.
- **`backend/.env.example`을 실제 운영 키 구성에 맞춤**: 기존 파일은 `DATABASE_URL`·`SECRET_KEY`·토스 항목만 있어 **로컬 개발용으로 오해되기 쉬웠는데**(개발은 루트 `.env`의 두 변수만 쓴다) 실제로는 운영 템플릿이었다. 운영 17개 키를 전부 덮고, 각 값이 잃었을 때 재발급 가능한지/복구 불가인지를 적었다. `RECOVERY.md` 시나리오 D에서 "무엇을 채워야 하는지"의 목록으로 쓰인다.
- **배운 것**: 헬스체크처럼 **당장 아무 일도 안 하는 것**을 지금 붙여두는 값이 있다. 이건 오늘 장애를 막지 않지만, ECS에서 태스크가 이유 없이 죽는 구간을 미리 없앤다. 반대로 `github-brench`처럼 **오늘 아무 일도 안 하고 있던 드리프트**는 오늘 사고를 이미 한 번 냈다 — 조용한 것이 안전한 것은 아니다.

### 🔎 2차 코드검사 — 감시가 스스로 눈먼 걸 초록으로 보고하고 있었다 (2026-07-22)

오늘 만든 것(watch·CI·배포 게이트·SSM 에스크로 등 커밋 8개)을 다시 검사했다. **치명 1건 + 중간 4건**이 나왔고 전부 내 코드다.

- **치명 — 경고가 아무에게도 안 갔다**: `watch.sh`의 알림 경로는 'Actions 실패 메일' 하나뿐인데 마지막 줄이 `[ "$FAIL" -eq 0 ]`이라 **`WARN`은 종료코드에 안 들어갔다**. 즉 경고로 처리한 것들은 초록으로 끝나고 신호가 0건이다. 하필 그 셋이 전부 '조용히 방치' 유형이었다 — ①6시간 넘게 켜둔 채 과금 중 ②이미지 사본 부족 ③**"SES 상태를 못 읽었다"**. 특히 ③이 치명적이다: `ses:GetAccount` 권한이 사라지면 **감시가 눈이 먼 상태**인데 초록으로 보고하고, SES 샌드박스를 4주간 아무도 몰랐던 그 상태로 정확히 되돌아간다. → 경고도 종료코드에 넣고, '못 읽었다'는 경고가 아니라 **실패**로 바꿨다.
- **중간 — 이미지 검사가 fail-open**: `aws s3 ls ... || true`가 '0건이면 exit 1'만 삼키는 게 아니라 **AccessDenied·자격증명 만료까지 삼켜서**, "권한이 없어 못 봤다"가 "볼 게 없다"로 보고됐다(초록). 07-22 IAM 드리프트 사고와 같은 계열이라 `list-objects-v2`로 바꿔 호출 실패와 0건을 구분한다.
- **중간 — 배포 게이트도 fail-open**: `git diff` 실패 시 `|| true` 때문에 조용히 통과. force push로 before가 dangling이 되면 `fetch-depth: 0`도 그 객체를 안 가져오므로 실제로 밟히는 경로다. 실패와 '변경 없음'을 구분해 fail-closed로. 감시 대상 경로도 `requirements.txt`·`Dockerfile`·`docker-compose.prod.yml`까지 넓혔다.
- **중간 — lifecycle 규칙이 또 헛돌고 있었다**: `newer_noncurrent_versions = 3`으로 바꾼 것도 **번들엔 안 걸렸다**. 번들은 content hash가 파일명에 있어 배포마다 키가 바뀌고, `sync --delete`가 옛 키에 삭제 표식을 얹으면 그 키의 옛 버전은 **1개**뿐이라 "3개 넘을 때만" 조건에 영원히 안 걸린다. → 실제 산출물 이름에 맞춰 `prefix = "index"`로. **적용 후 매치 3개 확인**(이전 규칙은 0개였다). `uploads/`는 규칙 밖이라 영구 보존이 이제 우연이 아니라 명시적이다.
- **중간 — 세 번째 사본이 실패해도 성공처럼 끝났다**: `push_ssm`이 `2>/dev/null`로 사유를 지우고 실패해도 `save`는 "보관 완료"로 exit 0. `check`도 SSM이 없거나 달라도 exit 0이라 "세 사본 대조"라는 이름값을 못 했다. 둘 다 종료코드에 반영. 덤으로 **SSM 대조를 서버 도달 검사보다 앞으로** 옮겼다 — 이 서버는 대부분 꺼져 있는데 예전 순서로는 그때 세 번째 사본을 아예 안 봤다.
- **그 밖**: 공개 API 검사가 HTTP 200만 보고 본문의 `mail`/`database`를 안 읽던 것(`status.py`를 STARTTLS+AUTH로 고쳐놓고 바깥에서 그 결과를 안 읽으면 두 장치가 안 이어진다), `terminated`를 '꺼짐'과 같이 취급하던 것, 날짜 파싱 실패 시 `set -u`가 스크립트를 끝내 나머지 검사가 날아가던 것, `.dockerignore`가 하위 폴더·`.env-bak` 같은 다른 구분자를 못 덮던 것, `SMTP_USER`가 비면 로그인을 건너뛰고 True를 돌려주던 것, 새 스크립트가 shellcheck 목록에서 빠지던 것.
- **배운 것**: 세 번 연속으로 **같은 실수를 다른 옷을 입고** 반복했다 — `prefix = "assets/"`(대상 0개) → `newer_noncurrent_versions`(대상 0개) → 그리고 `WARN`(알림 0건). 전부 "설정은 있는데 실제로는 아무 일도 안 한다"이고, 셋 다 **적용된 결과를 세어보고 나서야** 알았다. 규칙을 쓴 다음에는 **몇 개가 걸리는지 반드시 센다.**

### 🧾 3차 코드검사 — 내가 쓴 "마지막 드리프트"는 거짓이었다 (2026-07-22)
...
- **배포 정책을 최선으로**: 회수만 하고 내용은 그대로 뒀는데, 검사에서 두 가지가 나왔다. (1) `s3:GetObject`는 `aws s3 sync dist/ s3://...`에 **필요 없다**(올리기만 한다) → 제거하고 액션을 대상별로 쪼갰다. (2) 훨씬 중요한 것 — **업로드 이미지를 정책으로 못 박았다**:
  ...
  ```
  지금까지 복구 불가능한 이미지 전체를 지키는 건 워크플로의 `--exclude "uploads/*"` **한 줄**이었다. 명시적 Deny는 같은 정책의 Allow를 이기므로, 그 플래그를 빠뜨린 sync는 **삭제 대신 AccessDenied로 배포가 빨간불**이 된다 — 조용한 데이터 손실이 시끄러운 실패로 바뀐다. EC2의 이미지 업로드는 다른 주체(`blog-ec2-backup`)라 영향 없음을 확인했다. `db-backup.tf`가 EC2 쪽엔 이미 "버저닝이 되돌리는 쪽이면 이건 애초에 못 하게 하는 쪽"이라는 원칙을 적용해 뒀는데 **배포 쪽만 예외로 남아 있었다.**
- **헬스체크에 소비자를 붙였다**: `/api/health`는 무조건 200이라(DB를 안 본다) 이 체크가 말하는 건 "uvicorn이 응답한다"뿐이고, 저장소 전체에서 그 상태를 **읽는 곳이 0개**였다 = `docker ps` 장식. 그래서 ① `deploy_backend.sh`의 사후 확인이 **healthy가 될 때까지 기다리게** 했고 ② 개발 compose의 frontend `depends_on`을 `condition: service_healthy`로 바꿨다(운영엔 frontend가 없으니 **실제 소비자가 있는 곳은 개발 쪽이다** — 정작 거기 체크가 없었다). 타이밍도 고쳤다: `start_interval: 5s`를 넣어 기동 구간에선 5초마다 찔러 바로 healthy가 되게 하고 `start_period`를 120s로 늘렸다(빈 DB로 부팅하는 DR 때 마이그레이션 27개 + GIN 인덱스가 60초를 넘길 수 있다). `start_interval`이 조용히 무시되지 않는지 **파서로 확인**했다(bogus 필드는 거부, `start_interval`은 5s로 파싱됨). 로그 회전(`max-size 10m × 3`)도 같이 넣었다 — 30초마다 액세스 로그가 쌓이는데 그 디스크는 `pgdata`와 같은 EBS다.
- **`.gitignore`가 `.dockerignore`의 교훈을 안 배웠다**: 07-22에 `.dockerignore`를 `**/.env*`로 고쳐 시크릿 사본이 이미지에 구워지는 걸 막았는데, **깃 쪽은 안 고쳤었다.** `.env.bak`·`.env.save`·`.env-bak`이 전부 커밋 가능한 상태였다. **이미지보다 깃이 나쁘다 — 푸시하면 되돌릴 수 없다.**
- **env 템플릿을 '검증되는 계약'으로**: 템플릿이 낡아도 아무도 몰랐다(`env_escrow.sh check`는 세 사본의 **파일 해시**만 봤다). 키 **이름만** 대조하는 단계를 추가하고, 가짜 키를 넣어 실제로 잡히는지 음성 테스트까지 했다. 그 밖에 `ADMIN_EMAIL`이 06-25부터 죽은 키인 걸 명시, `PRO_PRICE_KRW`·`PRO_DAYS`가 돈 값인데 코드 기본값에만 사는 걸 명시, `SECRET_KEY` 회전이 세션만이 아니라 **이메일 인증·비번재설정·구독확인 링크까지** 죽인다는 걸 추가, DB 비밀번호 퍼센트 인코딩 함정 경고, 그리고 **이 템플릿을 `backend/.env`로 복사하면 앱이 안 뜬다**는 경고(리허설에서 실제로 겪은 재시작 루프의 원인이 바로 그 여분 키다).
- **배운 것**: 검사가 잡아낸 것 중 절반이 **버그가 아니라 내가 쓴 문장**이었다 — "마지막 드리프트", "최소권한", "ECS 대비". 코드는 돌지만 서술이 틀리면 다음 사람(대개 미래의 나)이 **다시 안 본다.** 오늘 세 번 반복한 "설정했는데 대상이 0개"의 사촌이다.

### 🔐 보안 검사 — 앱은 깨끗했고, 진짜 취약점은 가입 흐름에 있었다 (2026-07-22)

백엔드 전 소스·프론트·terraform·CI·스크립트를 훑고 AWS 읽기 조회와 OSV·npm audit로 교차 검증한 보안 검사. **찾아봤지만 없는 것부터**: SQL 인젝션, XSS, 경로 순회, 결제 금액 위변조, git 이력의 시크릿, 퍼블릭 S3, 파이썬 의존성 취약점 — 여섯 가지 다 **0건**이다. 앱 계층은 이 규모에서 이례적으로 잘 잠겨 있고, 남은 위험은 인프라 경계와 계정 운영에 있었다.

- **진짜 취약점 — 미인증 계정 선점(account pre-hijacking)**: `auth.py`의 재가입 분기가 인증 메일만 재발송하고 **비밀번호 해시를 갱신하지 않았다.** 공격자가 피해자 이메일로 먼저 가입해 두면(미인증), 피해자가 같은 주소로 가입할 때 인증 링크는 피해자에게 가지만 저장된 해시는 공격자 것으로 남는다 → 피해자가 링크를 누르는 순간 '검증된' 계정이 되는데 **로그인은 공격자만 할 수 있다.** 미인증 계정은 데이터가 없으므로 마지막 요청의 비번으로 덮어쓰는 게 안전하다. `token_version`도 올려 그전 링크를 무효화했다.
  - **회귀 테스트로 증명했다**: 수정을 되돌리면 실패하고 수정본에서는 통과한다(선점 → 재가입 → 인증 → 공격자 비번 401 / 피해자 비번 200). 116 → 117개.
- **SSRF — 리다이렉트로 검증을 우회**: `validate_base_url`이 저장·호출 시점에 스킴과 공인 IP를 검사하는데, openai SDK의 httpx 기본값이 `follow_redirects=True`라 **공격자 서버가 302로 내부 주소를 돌려주면 그대로 따라간다.** 검증은 최초 URL만 보고 최종 목적지는 못 본다. `http_client=httpx.Client(follow_redirects=False)`로 닫았다. (IMDS 자격증명 절도는 IMDSv2가 이미 막고 있음을 실측 확인 — 이 구멍으로 얻는 건 내부 포트 스캔 정도였다)
- **`/docs`·`/redoc`·`/openapi.json`이 열려 있었다**: CloudFront는 `/api/*`만 오리진으로 보내지만, 오리진 SG가 'CloudFront 엣지 **전체**'(prefix list `pl-22a6434b`)라 **공격자가 자기 CloudFront 배포를 만들어 직접 때리면** WAF·CSP·엣지 함수를 전부 우회하고 스키마가 통째로 노출된다. `docs_url=None`으로 껐다.
  - 정직하게, 그 경로로도 **레이트리밋은 안 뚫린다** — `ratelimit.py`가 XFF의 **마지막** 항목을 쓰는데 공격자의 CloudFront도 뷰어 IP를 끝에 붙이고 `X-Forwarded-For`는 커스텀 오리진 헤더로 설정할 수 없다. 그 설계는 의도대로 동작한다.
- **운영자 공인 IP가 공개 저장소에 박혀 있었다**: `ec2.tf`의 SSH 허용 대역이 실제 거주지 IP였다. `var.ssh_cidr` + `terraform.tfvars`(gitignore됨)로 뺐고, **기본값을 일부러 두지 않아** 값이 없으면 apply가 실패한다(0.0.0.0/0으로 조용히 넓어지는 것보다 낫다). `/32`만 허용하는 validation도 걸었다. ⚠️ **git 이력에는 남아 있다** — 되돌리려면 이력 재작성이 필요하다.
- **`.claude/settings.local.json`이 공개 저장소에 추적되고 있었다**: 이름 그대로 머신별 로컬 파일인데 작업 흐름·인프라 식별자·사전 승인 명령 목록이 노출됐다. gitignore + 추적 해제. 그리고 그 안의 `Bash(aws iam *)`·`Bash(aws ssm *)` **와일드카드**를 읽기 전용(`list-*`/`get-*`)으로 좁혔다 — 전자는 `iam delete-*`를, 후자는 `ssm put-parameter`(시크릿 쓰기)를 무프롬프트로 허용하고 있었다.
- **아직 안 고친 것(무거운 순)**: ① CloudFront→EC2 오리진이 **평문 HTTP:8000**이라 그 구간에 JWT·로그인 비밀번호·BYOK 키가 평문으로 흐른다. 커스텀 도메인+ACM이 필요해 로드맵이 비용 때문에 내려놓은 항목과 얽힌다. ② 오리진에 **공유 시크릿 헤더**가 없어 위의 '직접 오리진 접근'이 여전히 가능하다(코드 몇 줄 + terraform 한 블록이면 닫힌다). ③ **CloudTrail이 0개** — 감사 로그가 없어 사고가 나도 사후 추적이 불가능하다(첫 트레일은 무료, S3 저장만 월 몇 센트). ④ GitHub Actions 서드파티 액션이 SHA 핀이 아니다.
- **배운 것**: 검사가 "없다"고 확인해 준 목록이 발견 목록만큼 값지다. 앱 계층을 다시 파는 대신 **인프라 경계와 계정 운영**에 시간을 쓰면 된다는 뜻이고, 실제로 오늘 나온 것들이 전부 거기 있었다. 그리고 진짜 취약점 하나는 코드 리뷰로는 안 보이고 **흐름을 따라가야** 보였다 — "재가입하면 어떻게 되지"를 끝까지 밟아본 결과다.

### 🕵 CloudTrail — 감사 로그 0개였던 계정에 기록을 켰다 (2026-07-22)

보안 검사가 지적한 "이 계정에 CloudTrail·GuardDuty·Config가 전부 0개"를 메웠다. 관리자 키가 유출돼 백업 버킷이 지워지거나 SSM에서 시크릿이 통째로 읽혀도 **사후에 알 방법이 없던** 상태였다.

- **요금을 기억이 아니라 요금표로 확인하고 만들었다**: 관리 이벤트의 **첫 사본은 영구 무료**("one copy of your ongoing management events to your S3 bucket for free by creating trails" — 30일 체험인 CloudTrail Lake와 다르다). 돈이 붙는 **데이터 이벤트($0.10/10만)와 Insights($0.35/10만)는 일부러 켜지 않았고**, 적용 후 `get-event-selectors`로 데이터 셀렉터 0개·Insights 비활성을 실측 확인했다. 남는 건 S3 저장·요청뿐이고 이 계정 볼륨이면 월 몇 센트다.
- **전 리전 + 글로벌 서비스**: `is_multi_region_trail`과 `include_global_service_events`를 켰다. IAM·CloudFront·WAF는 us-east-1에 기록되므로 이게 없으면 정작 제일 중요한 "누가 IAM을 바꿨나"가 안 남는다. 리전당 첫 사본이 무료라 비용은 그대로다.
- **`enable_log_file_validation`**(무료)으로 해시 다이제스트를 같이 남겨 사후 변조를 탐지할 수 있게 했다.
- **버킷도 잠갔다**: 퍼블릭 차단 4종, SSE-AES256, 버저닝(감사 로그가 지워지면 감사가 안 된다), lifecycle 90일 + noncurrent 30일 + 삭제표식 정리로 무한 누적 차단.
- **버킷 정책에 `aws:SourceArn` 조건**을 걸었다. 조건이 없으면 다른 계정의 트레일이 우리 버킷에 로그를 밀어넣는 confused-deputy가 가능하다.
- **동작 확인**: `IsLogging: true`, 전송 오류 없음, 적용 직후 버킷에 객체가 실제로 생성됨.
