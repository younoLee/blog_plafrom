...
### 🛟 백업·복원 전면 보강 — 훈련이 통과시키던 13가지 [코드완료/적용대기] (2026-07-22)
...
- **아직 아닌 것**: `terraform apply`와 `env_escrow.sh save`는 **규칙7대로 사용자가 실행**. `RECOVERY.md`의 시나리오 B(인스턴스 유실 → 맨바닥 부트스트랩)는 **한 번도 밟아보지 않은 경로**다 — 재료가 다 있는지 확인해 쓴 절차이지 실증이 아니다. 그게 이 문서의 다음 할 일이다.
