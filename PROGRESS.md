...
- ❌ **당시 적은 "EIP는 붙어있는 동안 무료"는 틀림**(2024-02 이전 기준). 실제로는 ①모든 퍼블릭 IPv4가 시간당 $0.005 과금(EC2 프리티어 12개월은 월 750시간 무료라 지금은 $0), ②EIP가 **정지된** 인스턴스에 붙어 있으면 과금 대상. 07-17에 정정.
...
**2. 정지 상태 점검** — EC2·RDS 둘 다 `stopped` 확인. Cost Explorer 실측 **이번 달 $0**(전부 프리티어). ⚠️ **RDS는 정지 7일 후 자동 재시작**(AWS 정책) → 07-15 정지 기준 **~07-22에 스스로 올라옴**. 계속 꺼두려면 그때 재정지 필요.

**3. EIP 제거 + 오리진 주차 (핵심)**
- 07-15에 붙인 EIP를 제거. **동기는 비용이 아님**(프리티어 중 $0). 다만 제거하면서 **dangling origin** 위험이 드러남.
...
### 🔍 글 검색(pg_trgm) + 목록 페이지네이션 [로컬검증 완료 / ⚠️ 프로드 미배포] (2026-07-17)
...
- **남은 것**: 프로드 배포, UI 클릭 검증(브라우저 자동화 없어 로컬 http://localhost:5173/blog 에서 눈으로).
