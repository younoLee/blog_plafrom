...
```
- **크레딧: ~$75 남음 / 2027-06-24 만료** (콘솔 기준. 시작 $100 − 태운 $23.82 = $76.18로 산수가 맞는다).
- ⚠️ **`accountPlanRemainingCredits`는 잔액이 아니다** — 한 시간 사이 $115.10 → $135.10으로 **올랐다**(Lambda 생성이 활동 크레딧으로 잡힌 듯). 쓸수록 줄어야 할 값이 늘어나므로 **계획 근거로 쓰면 안 된다.** 잔액은 콘솔을 봐야 한다.
...
- **크레딧 수명 ($75 / 2027-06-24 만료 기준)** — **뭐가 먼저 바닥나는지가 사용 패턴에 따라 뒤바뀐다**:
  - **정지 유지**(월 ~$3.7, 스토리지만): 만료일까지 $42만 쓰고 **$33이 남은 채 소멸**. → 제약은 **만료일**이지 잔액이 아니다.
  - **24/7 가동**(월 ~$32): **2.3개월 = ~2026-09-26에 소진**. → 제약은 **잔액**.
- 🔑 **크레딧은 use-it-or-lose-it이다.** 2027-06-24에 사라지므로 **안 쓰고 남기는 데 이득이 없다.** "아껴서 오래 버티기"는 목표가 아니다 — 어차피 소멸할 $33을 지키느라 블로그를 못 띄우면 손해다. 진짜 목표는 **낭비(아무도 안 쓰는데 도는 것)만 없애고, 쓸 거면 만료 전에 쓰는 것**.
...
- **판단 보류**: 블로그를 언제 실제로 띄울지. 24/7이면 크레딧이 2.3개월(~09-26)이지만, 안 쓰면 $33은 그냥 소멸한다. **"켜두면 아깝다"가 아니라 "안 켜도 아깝다"**가 맞는 프레임.
