...
> ## ⚠️ 2026-07-31: 이 사유서는 지금 그대로 내면 안 된다 (보류)
>
> 아래 본문은 2026-07-22에 쓴 것이고, 그 뒤로 **논거 두 개가 사실이 아니게 됐다.**
>
> | 사유서의 주장 | 지금 |
> |---|---|
> | "방문자가 가입 폼을 제출하면 인증 메일을 보낸다" | **07-28에 초대제로 닫음** (`allow_signup=False` → 403) |
> | "뉴스레터 더블 옵트인으로 수신 동의를 받는다" | **07-31에 폐지** (확인해도 후속 메일이 없는 막다른 길이었다) |
>
> 이 상태로 제출하면 AWS에 사실과 다른 진술을 하는 것이고, 두 번째 거부를 부르기 좋다.
>
> **그리고 지금은 재신청이 필요한지 자체가 불분명하다.** 뉴스레터를 떼면서
> **검증 안 된 임의 주소로 메일이 나가는 경로가 0이 됐다.** 남은 메일은 전부
> 등록된 계정 주소로만 간다(비번 재설정 · 새 글 알림 · 가입 인증[초대제라 잠김]).
> 초대제 블로그의 계정 수는 유한하므로, **그 주소들을 SES ID로 검증하면
> 샌드박스인 채로 전부 동작한다** — 심사도, 기다림도 없다.
> (ID 검증 메일은 AWS가 직접 보내므로 샌드박스 수신 제한을 받지 않는다)
>
> **그래서 순서는 이렇다.**
> 1. 지금: 계정 주소를 SES ID로 검증한다 (재신청 불필요)
> 2. 나중에 **가입을 다시 열기로 결정하면**, 그때 그 결정을 근거로 사유서를
>    처음부터 다시 쓴다 — 아래 본문을 고쳐 쓰는 게 아니라.
>
> 아래는 참고용으로 남긴다. 형식(무엇을 얼마나 구체적으로 써야 하는가)은 여전히 유효하다.
