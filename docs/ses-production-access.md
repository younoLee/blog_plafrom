# SES 프로덕션 액세스 — **종결 (2026-08-04)**

> 이 문서는 닫혔다. 아래 절차·사유서는 **기록이지 할 일이 아니다.**
> 세 번째 신청은 하지 않는다. 이 앱은 **SES 샌드박스에서 계속 운영한다.**

## 결과: 두 번 신청, 두 번 거절

| 시점 | 경로 | 결과 |
|---|---|---|
| 2026-07-22 | SES 콘솔 (첫 신청) | **DENIED** · CaseId `178238423300607` |
| 2026-07-31 | Support 케이스 (Service limit increase) | **DENIED** (2026-08-04 회신) |

두 번째 회신은 정형 문구고 **사유를 밝히지 않는다** — "Due to security reasons, we are
unable to provide specific details about our assessment criteria."

**무엇이 걸렸는지 알려주지 않는 거절이라 고칠 대상이 없다.** 바뀐 사실 없이 다시 내면
같은 답이 온다. 그래서 재시도 대신 종결을 택했다.

## 종결이 성립하는 이유 — 샌드박스가 이 앱의 설계와 맞다

거절이 아프지 않은 건 우연이 아니라, 07-28·07-31에 이미 그쪽으로 정리해뒀기 때문이다.

- **가입이 초대제로 닫혀 있다** → 수신자 집합이 유한하다.
- **뉴스레터를 폐지했다**(`b4b4876`) → 검증 안 된 임의 주소로 나가는 경로가 **0**이다.
  남은 메일(가입 인증·비번 재설정·새 글 알림)은 전부 등록된 계정 주소로만 간다.
- 그 주소 3개가 **전부 SES에 검증돼 있다** — 샌드박스 수신 제한에 걸리지 않는다.

즉 프로덕션 액세스는 *있으면 좋은 것*이었지 *동작에 필요한 것*이 아니었다.

## 종결에 따른 확정 사항

| | |
|---|---|
| `watch.sh` | `SES_SANDBOX_EXPECTED=true` **영구히 그대로.** 바꿀 일이 없어졌다 |
| 가입 재개방 | **기각.** 캡차·6자리 코드는 승인을 전제로 한 계획이었다 → 만들지 않는다 |
| 초대제 | 임시 조치가 아니라 **결론**이다 |
| AWS에 한 약속 | 위반 아님 — "컨트롤 먼저, 그다음 오픈"의 전제가 승인이었다 |
| 새 수신자가 생기면 | `scripts/ses_verify_recipients.sh`로 그 주소만 검증하면 된다 |

## 사유 추정 (확인 불가 — 추정으로만 읽을 것)

AWS가 말을 안 하므로 아래는 증거 없는 추정이다. 다만 신청서에 실제로 들어간 사실 둘이
리뷰어가 통상 무겁게 보는 항목과 겹친다:

- **소유한 도메인이 없다.** 검증된 ID 3개가 전부 개인 메일 주소(gmail·naver)고,
  도메인 ID도 DKIM 서명도 없다. 발신자가 남의 도메인(gmail.com)인 셈이다.
- **Website URL이 `*.cloudfront.net` 기본 호스트명**이었다.

여기에 거절 이력이 있으면 정형 거절로 가는 게 자연스럽다.

## 이 결정을 뒤집을 유일한 조건 (지금 계획 없음)

도메인 취득 → SES **도메인 ID + DKIM** → 그 도메인으로 발신. 그때는 사실이 바뀌므로
다시 신청할 근거가 생긴다. 이건 백로그의 **'오리진 평문 HTTP:8000 (커스텀 도메인 + ACM
필요)'과 선행조건이 같아서** 도메인 하나로 둘이 같이 풀린다(연 $12~15, ACM 무료).
**SES 하나 때문에 사지는 않는다.** 다른 이유로 도메인을 사게 되는 날 딸려오는 보너스다.

---

# 이하는 기록 (2026-07-31 시점) — 실행하지 말 것
...
- ~~폐지된 뉴스레터 테이블 잔존 행 **4개(PII)** — 사유서와 무관하지만 정리 대상.
  ...
  > **(2026-08-10 정정) 이미 지워졌다 — `subscribers` 0행**(운영 DB 실측).
  > 2026-07-31에 지운 것인데 이 줄만 "아직 안 지웠다"로 열흘을 더 살았다.
  > 오늘 정정한 '안 한 일' 목록 여섯 건 중 일곱 번째다.
...
→ 2026-07-31에 지웠다(서버를 켜서 DB에서 직접).

⚠️ 처음엔 여기 "관리자 화면에서 지울 수 있다"고 적었는데 **그런 화면은 없다.**
`AdminPage.tsx`에 구독자 UI가 없고, `frontend/src/api/subscribers.ts`는 아무 데서도
import되지 않는 죽은 코드였다(그래서 07-31에 삭제). 백엔드의 관리자 라우트는 남아 있으니
정리는 API(curl)나 DB로 한다.
