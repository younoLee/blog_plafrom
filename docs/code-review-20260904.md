...
> ## ✅ 2026-09-05: 64건을 전부 처리했다
>
> 각 항목 머리에 그날 무엇을 했는지 인용구로 붙였고, 본문은 **발견 당시의 기록**으로
> 그대로 뒀다(무엇이 왜 그랬는지가 고침보다 오래 쓸모 있다).
>
> **2026-09-06: SEC-01 의 잔여도 닫혔다.** 남은 것은 하나다.
>
> - ~~**SEC-01 의 잔여**~~ — chunked 갈래가 본문을 파이썬 리스트가 아니라 스풀 파일로
>   받는다(512KB까지 메모리, 그 위는 디스크). 인증 헤더를 위조해도 프로세스가 지는
>   몫이 6MB → 512KB다. 자세한 것은 SEC-01 항목의 두 번째 인용구에 적었다.
> - **GAP-10 의 브라우저 e2e** — Playwright 설치가 필요한데 이 기계는 npm 원격
>   tarball 이 막혀 있다(`EALLOWREMOTE`). 화면 시험(결제 7 · 로그인 4)은 붙였다.
>
> 그리고 **일부러 안 한 것 넷**은 그대로다(WAF rate 룰 · 루트 볼륨 암호화 · 글마다 다른
> og:image · 열린 가입). 이유는 ROADMAP 과 개발일지에 적혀 있다.
>
> 덧붙인 것: 고치는 과정에서 **보고서가 못 본 것**이 둘 나왔다 — `compare_server_default`
> 를 켜자 드러난 실제 모델↔표 드리프트 둘(BQ-10 항목 참고), 그리고 이 문서 자신이
> OPS-8 절의 예시 주소 때문에 새 시크릿 검사에 걸린 것(OPS-8 항목 참고).
...
> ✅ **2026-09-05에 절반 했다.** 결제 화면 7개(해지 확인창이 남은 기간을 말하는지 ·
> 취소하면 요청이 안 나가는지 · 내역이 실패·대기를 보여주고 '확인 중'을 실패로 적지
> 않는지 · 영수증 링크)와 로그인 화면 4개(오류가 role="alert" 자리에 뜨는지 · 연타 가드 ·
> 라벨과 autoComplete · 성공 시 한 번만 호출)를 붙였다.
>
> **브라우저 e2e 는 못 했다.** Playwright 를 넣으려면 npm 설치가 필요한데 이 기계는
> 원격 tarball 이 막혀 있다(`EALLOWREMOTE`). 그래서 '안 한 것'이 아니라 **여기서는
> 할 수 없는 것**이고, 네트워크가 되는 곳에서 별도로 해야 한다.
...
> ✅ **2026-09-05에 만들었다.** `admin_actions` 표(마이그레이션 `c2d3e4f5a6b7`) +
> 일곱 라우트가 **같은 트랜잭션에** 한 줄씩 쓴다 + 관리자 화면의 '조치 기록' 절.
> **target 에 FK 를 안 건다** — 계정 삭제를 기록하는 게 주 용도라, FK 면 그 행이
> CASCADE 로 같이 지워지거나 누구였는지를 잃는다. id·이메일을 값으로 복사해 둔다.
> 시험 다섯 중 하나는 **400 으로 막힌 조치가 기록되지 않는지**를 본다(같은 트랜잭션이라
> 롤백된다 — 표가 '일어나지 않은 일'을 말하면 안 된다).
...
> ✅ **2026-09-05에 만들었다.** 승인 시 인앱 알림(같은 커밋)과 푸시(백그라운드)를 낸다.
> 종류를 가르려고 `notifications.kind` 를 더했다(마이그레이션 `b1c2d3e4f5a6`, 옛 행은
> `subscribe_request` 로 백필). 모델이 '종류를 열로 두지 않는다'고 적어둔 규칙에 **예외를
> 하나 둔 것**이라, 왜 여기만 예외인지를 모델·마이그레이션 양쪽에 적었다 — 신청과 승인은
> post_id NULL + actor_id 로 모양이 완전히 같고, 방향으로 가르려면 매번
> author_subscriptions 를 조인해야 하는데 구독을 취소하면 그 조인이 답을 못 준다.
> 승인은 멱등이다(두 번 눌러도 알림 한 줄). 거절은 일부러 안 알린다.
...
> ✅ **2026-09-05에 만들었다.** `GET /api/subscriptions/subscribers`(승인된 사람만) +
> 구독 화면의 '내 구독자' 절과 해지 버튼(확인창 포함 — 되돌리려면 재신청·재승인이 필요하다).
> 시험 다섯 중 하나는 **두 목록이 서로 배타적인지**를 본다(겹치면 같은 사람이 '대기중'과
> '구독자'로 두 번 뜬다).
...
> ✅ **2026-09-05에 만들었다.** 삭제 조건에 본인을 더하고 `PATCH /{comment_id}`(본인만,
> 내용만)를 새로 뒀다. 응답에 `is_mine` 1비트를 실어 화면이 '고치기·지우기'를 그린다
> (user_id 는 그대로 안 내보낸다 — 그건 신원이고 이건 보는 사람 자신에 대한 1비트다).
> **글쓴이·관리자에게 수정은 안 연다** — 남의 말을 지우는 것(모더레이션)과 남의 말을
> 고치는 것은 다른 권한이다. 익명 댓글은 소유를 증명할 방법이 없어 둘 다 안 된다.
...
> ✅ **2026-09-05에 만들었다.** `GET /auth/me/export`(계정·글·회원으로 단 댓글 JSON)와
> `POST /auth/me/delete`(비밀번호 재확인) + 설정 화면의 '계정' 절. **둘을 한 자리에 둔다** —
> 가져갈 방법이 없는 삭제는 '전부 잃거나 전부 남기거나'만 남겨서 실제로는 아무도 못 지운다.
> DELETE 가 아니라 POST 인 이유는 비밀번호를 본문으로 받기 때문이다(SEC-07 과 같은 규칙).
> 관리자 계정은 스스로 못 지운다 — 사라지면 아무도 관리 화면에 못 들어간다.
> 익명 댓글은 내보내기에 없다(user_id 가 없는 게 익명의 정의다).
...
> ✅ **2026-09-05에 만들었다.** 넷 다 했다 — ① 승인 응답의 `receipt.url` 저장
> (마이그레이션 `d3e4f5a6b7c8`), ② `GET /payments/me` 와 `GET /admin/payments`
> (실패·대기 주문도 보여준다 · '확인 중'을 실패로 적지 않는다), ③ 해지 확인창이
> **남은 N일이 바로 사라지고 환불이 없다**고 말한다, ④ 만료 7일 전부터 연장 결제 허용
> (`RENEW_WINDOW_DAYS`). 영수증 저장은 **없어도 확정이 되는지**까지 시험한다 —
> 거기서 죽으면 돈은 나갔는데 확정이 안 된다.
...
> ✅ **2026-09-05에 만들었다(관리자 쪽).** `PATCH /admin/users/{id}/email` + 화면 버튼.
> 보고서가 권한 대로 **관리자 경로**를 골랐다 — 본인이 바꾸려면 새 주소로 확인 링크를
> 보내 소유를 증명받아야 하는데, SES 샌드박스는 검증된 주소로만 보내므로 그 링크가
> 새 주소에 영영 안 닿는다(초대제가 '주소는 관리자가 고른다'로 설계된 것과 같은 제약).
> 바꾸면 `token_version` 을 올려 기존 세션을 끊고, 조치는 감사 기록에 남는다.
...
> ✅ **2026-09-06에 그 남은 숙제를 닫았다.** chunked 갈래의 `buffered: list[dict]` 를
> `SpooledTemporaryFile(max_size=512KB)` 로 바꿨다. 문턱까지는 메모리, 그 위는 디스크다.
> 헤더를 위조해 6MB를 밀어넣어도 프로세스 메모리에 남는 것은 512KB이고, 무인증 JSON
> 경로는 **상한 자체가 512KB라 한 번도 디스크에 닿지 않는다** — 늘어난 파일 I/O는
> 업로드 경로뿐이다.
>
> 되돌려주기(replay)에서 두 가지를 지켰다. ① 조각 경계는 256개까지 그대로 재현한다
> (정상 트래픽은 전부 여기 들어오므로 하위 앱이 보는 모양이 안 바뀐다). ② 그 위로는
> 경계를 버리고 64KB로 잘라 준다 — 1바이트짜리 조각으로 6MB를 보내면 **경계 목록
> (int 600만 개)이 본문보다 커져서** 디스크로 옮긴 의미가 사라지기 때문이다. 고치면서
> 새로 생긴 함정이라 시험으로 못박았다(`test_many_tiny_chunks_do_not_keep_a_boundary_per_chunk`).
>
> 임시파일은 `finally` 에서 닫는다(닫기 = 삭제). 안 닫으면 고장이 없어진 게 아니라
> 메모리에서 디스크로 자리를 옮긴 것이 되므로, 앱이 예외를 던지는 경우까지 시험이 있다.
> 시험 일곱을 붙였다 — 문턱 위/아래 · 임시파일 정리 둘 · 조각 합치기 · 부분 본문 뒤
> disconnect 순서 · 디스크를 경유한 1MB multipart 업로드가 그대로 파싱되는지.
