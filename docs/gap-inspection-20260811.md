...
| 항목 | 결과 |
|---|---|
| **개발일지 #27·#28 발행** | id 39·40, 공개 글 28건, 연재 네비 28/28 실측 |
| 프론트 사용자 공백 | 홈 로딩 단언·글 상세 잔상·로그인 무반응·글 중복 생성·구독 60초·관리자 죽은 게이지 |
| 접근성·모바일 | 알림 드롭다운 Escape+aria · `prefers-reduced-motion`(대응 0건이었다) · 관리자 행 넘침 |
| 검색 스니펫 | 불릿이 이어붙어 비문이던 것(28편 중 9편) → `·` 구분자 |
| `cleanup.py` 로깅 | 성공 건수 + `logger.exception`. 백그라운드 스레드라 예외가 그냥 사라지던 자리 |
| **감시 하트비트** | `blog/watch HeartBeat` + `treat_missing_data="breaching"` 알람. 무료 한도 안이라 **비용 0** |
| **`blog-app-secrets` 삭제** | `enable_ecs` 게이트를 씌워 destroy(30일 복구 창) |
| **AI 전체 상한 + 토큰 계량** | 아래 |

### AI 토큰 — 이 저장소에서 처음으로 토큰을 센다

**돈이 나가는 건 서버키(Claude) 경로 하나**뿐이라 거기만 계량한다. BYOK는 사용자 본인
청구고, 벤더 4곳의 usage 구조를 다 좇는 유지비는 값을 못 한다.

- `_claude` → `(텍스트, TokenUsage|None)`. `generate_draft`도 같은 모양(계약이 바뀌었으므로
  테스트 목 5곳도 같이 고쳤다 — 숨기지 않는다)
- `ai_usage`에 `input_tokens`/`output_tokens` + 마이그레이션 `e1f2a3b4c5d6`.
  `server_default="0"`이 중요하다 — 기존 행이 NULL이면 SUM이 NULL이 되고 상한 비교가
  조용히 통과한다(fail-open)
- **서비스 전체 상한 둘**: 횟수 60/일 · 토큰 60만/일. 지금까지 캡이 **전부 user_id 단위**라
  계정이 늘면 총액에는 상한이 없었다
- **usage가 없으면 0이 아니라 '모름'이다.** 0으로 세면 토큰 상한이 조용히 무력화되므로
  기록도 안 하고, 그땐 횟수 상한이 받쳐준다. 테스트로 잠갔다

**여전히 안 한 것**: 이 숫자를 관리자 화면에 띄우는 것(지금은 DB·컨테이너 로그에 있고
상한이 강제한다) · 글별 OG 태그(SPA에서 JS로 바꿔봐야 공유 스크래퍼는 JS를 실행하지
않는다 — 진짜 해법은 정적 아카이브를 DB 글까지 넓히는 것이다) · `posts.owner_id`의 `ondelete`.

---

**1차에서 안 했던 것** — 프론트 사용자 공백 10건(로딩 상태·OG 태그·접근성·모바일),
