...
```
# 1) 네트워크·RDS·시크릿 컨테이너만 먼저 (모든 apply에 이미지 태그 필요)
terraform apply -var="backend_image_tag=<SHA>" \
  -target=aws_security_group.rds -target=aws_security_group.task -target=aws_security_group.alb \
  -target=aws_db_instance.main -target=aws_secretsmanager_secret.app
# 2) 앱 시크릿 4키 채우기(선택키도 최소 빈 문자열)
aws secretsmanager put-secret-value --secret-id blog-app-secrets \
  --secret-string '{"SECRET_KEY":"...","ANTHROPIC_API_KEY":"...","LLM_ENCRYPTION_KEY":"...","TOSS_SECRET_KEY":"..."}'
# 3) 데이터 이관(아래 B) → 4) 전체 apply(ECS·ALB 기동)
terraform apply -var="backend_image_tag=<SHA>" -var="backend_origin_dns=<현재 EC2 DNS>"
```
**모든 apply에 `-var="backend_origin_dns=<현재 EC2 DNS>"`를 준다** — 안 주면 기본 ""가 오리진을 S3로
**주차**해 `/api`가 fail-closed(라이브 다운)된다. (ecs 컷오버 전까지 EC2가 계속 서빙해야 하므로)

**B. RDS 복원은 dump를 손봐야 한다(안 그러면 빈 DB).**
- 컨테이너 Postgres의 `postgres`는 슈퍼유저라 dump에 `COMMENT ON EXTENSION plpgsql`이 들어있는데,
  RDS 마스터는 슈퍼유저가 아니라 그 줄에서 `must be owner of extension` → `ON_ERROR_STOP`이면 **첫 줄에서 중단**.
  → 그 줄을 걸러 복원: `gunzip -c dump.sql.gz | grep -v '^COMMENT ON EXTENSION plpgsql' | psql -h <rds> -U postgres -d blog`
- **전체 dump만 복원한다. 복원 전에 `alembic upgrade head`를 돌리지 마라** — dump에 이미 스키마+pg_trgm+
  alembic_version이 다 있어서, 먼저 alembic을 돌리면 복원이 `relation already exists`로 깨진다.
  (서빙 command엔 alembic이 없으니 복원만으로 스키마·pg_trgm·GIN이 다 온다. 마이그레이션 원샷은 이관 후 무해한 no-op.)
- 복원 후 **`/api/health` 말고 DB경로(`/api/status`·`/api/posts`)로 스모크** — health는 빈 DB여도 200이다.

**C. 롤백·정리도 안전판이 필요.**
- 롤백 `terraform apply -var="api_backend=ec2"`도 **`-var backend_image_tag=<SHA>` + `backend_origin_dns`**가
  있어야 동작(없으면 required-var 에러). EC2가 정지돼 있었다면 먼저 인스턴스 start.
- Stage 7 정리는 **targeted destroy**로(공유 config라 `terraform destroy`는 CloudFront·EC2·S3까지 다 지운다).
  먼저 `api_backend=ec2`로 오리진을 EC2로 돌린 뒤 ALB/ECS/RDS만 destroy.

**비용 정정:** desired_count=2(항상 2태스크) 기준 실측 ≈ **월 $52** (ALB~16 + Fargate2대~20 + RDS~13 +
스토리지~2.3 + Secrets~0.8). 계획 초안의 $30~35보다 높다. 예산이 빡빡하면 `min_capacity=1`로 반감.

**3차 검사 고친 코드(위 apply와 별개, 백엔드 배포 시 적용):** toggle_pro가 pro_until을 안 지워 관리자
Pro 부여가 되돌려지던 것, 동시 중복삽입 500 4곳(구독·구독me·공개구독·BYOK키등록)을 멱등 처리.
**남긴 LOW(후속):** ①알림 목록이 비공개 전환된 글 제목을 노출(재검사 필요) ②`posts.created_at` 인덱스
없음(정렬 핫패스, 마이그레이션 필요) ③`ai_hourly_usage`·`status_checks` 무한 증가(보관정책 없음).
검증됨(깨끗): SSRF/BYOK 격리, IDOR/authz, JWT, 마이그레이션↔모델, 타임존, SQLi — 신규 결함 없음.
...
- **준비물 완성(무비용):** 부하 스크립트 `scripts/loadtest.k6.js`(오토스케일 유발, AI엔드포인트 제외·
  읽기경로만) + 캡처 체크리스트 `docs/ecs-stage6-prove.md`(시연→캡처→면접 한 줄, 가용성 프로버·정리 포함).
- 캡처·로그로 남길 것: **무중단 롤링 배포** / **태스크 강제 종료 → ECS 자동 대체** /
  **오토스케일**(CPU 타깃 트래킹으로 부하 시 스케일아웃) / **ALB 헬스체크가 unhealthy 태스크 축출** /
  CloudWatch 지표·로그. → 상세 절차·명령은 `docs/ecs-stage6-prove.md`.
- 실비용 ~$1 이하(시간당 ~$0.08). 한나절이면 충분. 개발일지 작성 = "오케스트레이션" 서사가 증거를 얻는 지점.
