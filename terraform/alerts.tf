...
# ── 감시가 돌았는데 문제를 보고했다 ─────────────────────────────────────────
# 바로 위 하트비트 알람과 **짝**이다. 그쪽은 "감시가 안 돈다"를 보고, 이쪽은
# "감시는 돌았는데 뭔가 잡혔다"를 본다. 둘을 갈라놓는 이유는 대응이 다르기 때문이다 —
# 전자는 워크플로를 살려야 하고, 후자는 잡힌 항목을 고쳐야 한다.
#
# **왜 만들었나 (2026-09-07).** `watch.yml` 머리말이 "실패하면 GitHub이 메일로 알려준다"고
# 적어두고 그걸 유일한 알림 경로로 삼고 있었다. 2026-09-06 밤에 감시가 빨간불이 됐는데
# **다음 날 사람이 Actions 탭을 직접 열어보기 전까지 아무도 몰랐다.** 그 문장이 사실인지
# 아무도 확인한 적이 없었던 것이다. 07-30 비용 훈련의 결론("발동했다와 닿았다는 다르다")을
# 정작 감시 자신에게는 적용하지 않고 있던 자리다.
#
# **왜 새 경로를 안 만들었나.** 이 SNS 토픽은 `watch.sh` 가 매 실행 "알람 전달 경로 정상
# (확인된 구독 N개)"으로 되짚고 있는, **사람에게 닿는 것이 확인된 유일한 경로**다.
# 새 채널을 만들면 그 채널이 닿는지를 또 확인해야 한다. 그리고 지표 네임스페이스가
# 하트비트와 같아서 감시 역할의 **IAM 을 안 건드린다**(PutWatchHeartbeat 가
# `blog/watch` 네임스페이스 단위로 열려 있다).
#
# 값은 `FAIL + WARN` 이고 이는 watch.sh 종료코드와 **같은 식**이다. 경고만 있어도 울린다 —
# 09-06 의 그 빨간불(프론트가 2커밋 뒤처짐)도 경고 1건이었고, 그건 알아야 하는 일이었다.
#
# `treat_missing_data = "notBreaching"` — 데이터 없음은 '감시가 안 돎'이고 그건 위
# 하트비트 알람의 몫이다. 여기서 breaching 으로 두면 **같은 사건에 메일이 두 통** 간다.
# (하트비트 알람과 정확히 반대로 설정하는 자리라 헷갈리기 쉬워 적어둔다.)
#
# 창을 1시간·1회로 잡은 근거: 이 지표는 감시가 돌 때만 찍히고 그 간격이 실측 1~5시간이다
# (watch.yml 의 스케줄 주석). 즉 "한 시간 안에 데이터포인트 하나"는 사실상 **실행 한 번**을
# 뜻하고, 문제를 본 실행이 한 번이라도 있으면 바로 울린다. 값이 0 으로 돌아오면 스스로 꺼진다.
#
# 비용: 커스텀 지표 1 + 알람 1 추가. 상시 무료 한도가 지표 10 · 알람 10이고 이 계정은
# 지표 1 · 알람 3 이었다 → **추가 비용 0.**
resource "aws_cloudwatch_metric_alarm" "watch_problems" {
  alarm_name        = "blog-watch-problems"
  alarm_description = "감시(GitHub Actions)가 실행됐고 실패·경고를 보고했다. Actions 탭의 '감시' 최근 실행 로그를 열어 잡힌 항목을 볼 것."
  ...
  threshold           = 1
  treat_missing_data  = "notBreaching" # 위 주석 — 침묵은 하트비트 알람의 몫이다

  alarm_actions = [aws_sns_topic.alerts.arn]

  # ok_actions 를 붙인다 — 위 알람들과 다른 선택이라 이유를 적는다.
  # 이 알람은 '사람이 고쳐야 할 항목이 있다'는 뜻이고, 고치면 다음 실행에서 0 이 된다.
  # 그 복구는 **감시가 실제로 다시 돌아 0 을 찍었다**는 뜻이라 거짓말을 하지 않는다
