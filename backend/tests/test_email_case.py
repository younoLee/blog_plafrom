...
2026-08-09에 `uq_users_email_lower`로 닫았다.

**이 파일이 검증하는 범위를 정확히 적어둔다.** conftest는 스키마를
`Base.metadata.create_all`로 만든다 — 즉 아래 테스트들은 **모델의 `__table_args__`가
실제 DDL이 되는지**까지만 본다. 마이그레이션은 한 줄도 안 탄다.
처음엔 이 자리에 "인덱스가 DDL에 나갔는지 본다"고만 썼는데, 어느 DDL인지가 빠져
있어서 모델↔마이그레이션 짝까지 지키는 것처럼 읽혔다(2026-08-09 코드검사가 잡았다).

모델과 마이그레이션이 갈라지는 것은 **CI의 '빈 DB에 alembic upgrade head' 잡**이 본다.
그게 없으면 로컬은 초록이고 프로드 기동에서 처음 드러난다 — 2026-08-07에 그렇게 당했다.
"""
import pytest
