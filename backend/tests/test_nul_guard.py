...
CASES = [
    ...
    # ⚠️ **series 는 2026-09-05까지 이 목록에 없었다**(09-04 검사 BQ-6). 코드 쪽
    # (posts.py 의 `has_nul(q, tag, author, series)`)은 08-27에 넷을 다 막았는데,
    # '무인증 입구를 전부 훑는다'고 선언한 이 목록만 셋에 머물러 있었다 —
    # 위 세 줄이 적어둔 '짝지어진 것 중 한쪽만 안 쓸린' 모양의 네 번째다.
    (f"/api/posts?series={NUL}", 422),
]
