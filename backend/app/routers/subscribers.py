...
**2026-08-27 실측: 운영 DB의 `subscribers` 행이 0개다.** 즉 위 문단이 걱정한 '이미
쌓인 주소'는 없다. 쓰는 코드도 저장소에 하나도 없으므로(이 파일의 두 라우트 외에는
모델 정의뿐이다) 이 테이블은 0행에 고정돼 있고 **늘어날 수도 없다.**

그런데 이 두 라우트를 부르는 화면도 없다 — `frontend/src/api/`에 `subscribers.ts`가
아예 없다. 만들어놓고 입구를 안 뚫은 상태이고, 감사가 이걸 지적했다.

그래서 결론은 '정리 UI를 만든다'가 아니라 **'테이블과 함께 없앤다'**인데, 그건
마이그레이션이라 여기서 하지 않는다. 라우터만 먼저 지우면 테이블에 닿는 길이
사라지므로 순서도 그쪽이 맞다. 남은 일은 하나다:

  · `subscribers` 테이블 drop 마이그레이션 + 이 파일·모델·스키마 제거

그때까지 이 두 라우트는 **화면 없는 채로 둔다.** 0행을 실측해 적어뒀으니 다음 사람이
같은 조사를 다시 할 필요는 없다.
