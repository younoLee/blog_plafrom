...
class Settings(BaseSettings):
    ...
    # 막힌다(밖에서도 그대로 403 이다).
    #
    # ⚠️ **2026-09-05까지 이 자리에 "밖에서는 403이 아니라 200+HTML로 보인다"가 적혀
    # 있었다**(09-04 검사 SEC-05). 그 서술의 근거였던 블록은 2026-07-28에 제거됐고,
    # 08-10 보안검사가 같은 거짓을 RECOVERY.md·variables.tf 와 함께 "셋을 고쳤다"고
    # 적었는데 **이 파일만 안 고쳐졌다.** 하필 origin_secret 을 켜다 막혔을 때 사람이
    # 제일 먼저 여는 파일이 여기다 — 런북이 재해 중에 없는 증상을 찾게 만드는 그 모양
    # (첫 DR 게임데이의 42분 중 20분이 문서가 틀린 자리였다).
